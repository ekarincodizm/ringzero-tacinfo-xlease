...
function page(id,s){
    $("#panel").fadeOut(300);
    $("#panel").load("car_insure_un_panel.php?id="+ id +"&s="+ s);
    $("#panel").fadeIn(300);
}
</script>
...
<input type="button" name="bb1" id="bb1" value="ประกันภัยภาคบังคับ (พรบ.)" class="ui-button" onclick="location='car_insure_panel.php?id=<?php echo $_GET['id']; ?>'">
<input type="button" name="bb2" id="bb2" value="ประกันภัยภาคสมัครใจ" class="ui-button" onclick="location='car_insure_un_panel.php?id=<?php echo $_GET['id']; ?>'" disabled>
</div>
<div style="float:right" class="title_top">ประวัติซื้อประกันของรถ</div>
...
$id = $_GET['id'];
$s = $_GET['s'];
$qry=pg_query("select * from insure.\"InsureUnforce\" WHERE \"CarID\" = '$id' ORDER BY \"InsUFIDNO\" ASC");
$numrows = pg_num_rows($qry);
if($numrows == 0){ echo "ไม่พบประวัติการซื้อประกัน !"; exit; }
while($res=pg_fetch_array($qry)){
    $InsFIDNO[] = $res["InsUFIDNO"];
}
...
<?php
    ...
    WHERE A.\"InsUFIDNO\"='$InsFIDNO[$search_array]'");
...
$qry_vcus=pg_query("select * from \"FOtherpay\" WHERE  \"RefAnyID\"='$InsFIDNO[$search_array]' AND \"O_Type\"='102' AND \"Cancel\"='false' order by \"O_DATE\"  ");
