...
$pdf->SetXY(100,$cline); 
$buss_name=iconv('UTF-8','windows-874',number_format( ($aroverdue-$uroverdue) - ((($aroverdue-$uroverdue)*$security)*$reserve),2));
$pdf->MultiCell(25,4,$buss_name,0,'R',0);

$pdf->SetXY(125,$cline);
$buss_name=iconv('UTF-8','windows-874',number_format( ($arnextydue-$urnexty) - ((($arnextydue-$urnexty)*$security)*$reserve),2));
$pdf->MultiCell(25,4,$buss_name,0,'R',0);

$pdf->SetXY(150,$cline);
$buss_name=iconv('UTF-8','windows-874',number_format( ($arotherydue-$urothery) - ((($arotherydue-$urothery)*$security)*$reserve),2));
$pdf->MultiCell(25,4,$buss_name,0,'R',0);

$pdf->SetXY(175,$cline);
$buss_name=iconv('UTF-8','windows-874',number_format( ($artotal-$urtotal) - ((($artotal-$urtotal)*$security)*$reserve),2));
$pdf->MultiCell(24,4,$buss_name,0,'R',0);
