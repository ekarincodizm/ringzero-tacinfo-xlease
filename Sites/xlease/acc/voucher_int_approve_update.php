...
<div style="float:left"><input name="button" type="button" class="ui-button" onclick="window.location='voucher_int_approve.php'" value="ย้อนกลับ" /></div>
<div style="float:right"><input type="button" value="  Close  " class="ui-button" onclick="javascript:window.close();"></div>
<div style="clear:both"></div>

<fieldset><legend><B>Internal Approve Voucher</B></legend>
...
for($i=0;$i<$count_select;$i++){
    ...
    if($exp_select[0] == "L" && isset($_POST['button2']) ){
        ...
        $result=pg_query("insert into account.\"IntAccHead\" (\"type_acb\",\"acb_id\",\"acb_date\",\"acb_detail\",\"ref_id\") values ('GJ','$res_gj_id','$now_date','ยกเลิกรายการ $exp_select[1]','$exp_select[1]');");
        if(!$result){
            $status += 1;
        }
        ...
        $qry_name2=pg_query("SELECT * FROM account.\"IntAccDetail\" WHERE \"RefID\" = '$exp_select[1]' ORDER BY \"auto_id\" ASC");
        while($res_name2=pg_fetch_array($qry_name2)){
            $AcID = $res_name2["AcID"];
            $AmtDr = $res_name2["AmtDr"];
            $AmtCr = $res_name2["AmtCr"];
            
            $result=pg_query("insert into account.\"IntAccDetail\" (\"autoid_abh\",\"AcID\",\"AmtDr\",\"AmtCr\",\"RefID\") values ('$res_auto_id','$AcID','$AmtCr','$AmtDr','$exp_select[1]');");
            if(!$result){
                $status += 1;
            }
