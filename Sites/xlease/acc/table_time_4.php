...
<input name="button" type="button" onclick="window.location='table_time_4.php?p=1&yy=<?php echo $yy; ?>'" value="แสดงเป็นรายบุคคล" <?php if($p==1) { echo "disabled"; }?> />
<input name="button" type="button" onclick="window.location='table_time_4_s.php?p=2&yy=<?php echo $yy; ?>'" value="แสดงเป็นสรุปประจำปีลูกหนี้" <?php if($p==2) { echo "disabled"; }?> />
</div>
...
<table width="100%" border="0" cellSpacing="1" cellPadding="2" align="center" bgcolor="#F0F0F0">
    <tr style="font-weight:bold;" valign="middle" bgcolor="#79BCFF">
        ...
        <td align="center" rowspan="2">Reserve</td>
        <td align="center" colspan="4">Bad Debts Reserved</td>
    </tr>
    <tr style="font-weight:bold;" valign="middle" bgcolor="#79BCFF">
...
<?php
    ...
    $s_aroverdue += (($aroverdue-$uroverdue)*$security)*$reserve;
    $s_arnextydue += (($arnextydue-$urnexty)*$security)*$reserve;
    $s_arotherydue += (($arotherydue-$urothery)*$security)*$reserve;
    $s_artotal += (($artotal-$urtotal)*$security)*$reserve;
    $s_aroutstanding += (($aroutstanding-$urtotal)*$security)*$reserve;

    $s_aroverdue_all += (($aroverdue-$uroverdue)*$security)*$reserve;
    $s_arnextydue_all += (($arnextydue-$urnexty)*$security)*$reserve;
    $s_arotherydue_all += (($arotherydue-$urothery)*$security)*$reserve;
    $s_artotal_all += (($artotal-$urtotal)*$security)*$reserve;
    $s_aroutstanding_all += (($aroutstanding-$urtotal)*$security)*$reserve;
...
?>
        ...
        <td align="center"><?php echo $reserve; ?></td>
        <td align="right"><?php echo number_format((($aroverdue-$uroverdue)*$security)*$reserve,2); ?></td>
        <td align="right"><?php echo number_format((($arnextydue-$urnexty)*$security)*$reserve,2); ?></td>
        <td align="right"><?php echo number_format((($arotherydue-$urothery)*$security)*$reserve,2); ?></td>
        <td align="right"><?php echo number_format((($artotal-$urtotal)*$security)*$reserve,2); ?></td>
    </tr>
...
<tr bgcolor="#ffffff" style="font-size:11px; font-weight:bold;">
    <td align="left" colspan="2"><a href="table_time_4_print.php?yy=<?php echo $yy; ?>" target="_blank"><img src="icoPrint.png" border="0" width="17" height="14"> พิมพ์รายงาน</a></td>
