...
<div style="float:left"><input name="button" type="button" onclick="window.location='voucher_int_change.php'" value="ย้อนกลับ" /></div>
...
//Head
$result=pg_query("insert into account.\"IntAccHead\" (\"type_acb\",\"acb_id\",\"acb_date\",\"acb_detail\",\"ref_id\") values ('GJ','$res_gj_id','$now_date','$text_insert','$vcid');");
if(!$result){
    $status += 1;
}
...
//Detail
$qry_name2=pg_query("SELECT * FROM account.\"IntAccDetail\" WHERE \"RefID\" = '$vcid' ORDER BY \"auto_id\" ASC");
while($res_name2=pg_fetch_array($qry_name2)){
    ...
    if($sumall > $cash_change){
        if($AmtDr == 0){
            $result=pg_query("insert into account.\"IntAccDetail\" (\"autoid_abh\",\"AcID\",\"AmtDr\",\"AmtCr\",\"RefID\") values ('$res_auto_id','$AcID','$change_money_abs','0','$vcid');");
        }elseif($AmtCr == 0){
            $result=pg_query("insert into account.\"IntAccDetail\" (\"autoid_abh\",\"AcID\",\"AmtDr\",\"AmtCr\",\"RefID\") values ('$res_auto_id','$AcID','0','$change_money_abs','$vcid');");
        }
    }elseif($sumall < $cash_change){
        if($AmtDr == 0){
            $result=pg_query("insert into account.\"IntAccDetail\" (\"autoid_abh\",\"AcID\",\"AmtDr\",\"AmtCr\",\"RefID\") values ('$res_auto_id','$AcID','0','$change_money_abs','$vcid');");
...
//Head
$result=pg_query("insert into account.\"IntAccHead\" (\"type_acb\",\"acb_id\",\"acb_date\",\"acb_detail\",\"ref_id\") values ('GJ','$res_gj_id','$now_date','รายการรับเข้า ลงบัญชี $vcid','$vcid');");
if(!$result){
    $status += 1;
}
$auto_id=pg_query("select currval('account.\"IntAccHead_auto_id_seq\"');");
...
//Detail
$result=pg_query("insert into account.\"IntAccDetail\" (\"autoid_abh\",\"AcID\",\"AmtDr\",\"AmtCr\",\"RefID\") values ('$res_auto_id','$select_type','$cash_change','0','$vcid');");
if(!$result){
    $status += 1;
}
$result=pg_query("insert into account.\"IntAccDetail\" (\"autoid_abh\",\"AcID\",\"AmtDr\",\"AmtCr\",\"RefID\") values ('$res_auto_id','$vender_acid','0','$cash_change','$vcid');");
if(!$result){
    $status += 1;
}
//จบ Insert Acc ----------------------------------------------------------------------
...
<input name="button" type="button" onclick="javascript:window.open('voucher_change_print.php?id=<?php echo "$vcid"; ?>' , 'P32424224554M<?php echo "$vcid"; ?>','menuber=no,toolbar=yes,location=no,scrollbars=no, status=no,resizable=no,width=800,height=600')" value="พิมพ์" />
