...
<div style="float:left"><input name="button" type="button" onclick="window.location='fvoucher_payment.php'" value="ย้อนกลับ" /></div>
<div style="float:right"><input type="button" value="  Close  " onclick="javascript:window.close();"></div>
<div style="clear:both"></div>

<fieldset><legend><B>Voucher Pay</B></legend>
...
if($chkbuy == 1){
    $insert_voucher="insert into account.\"voucher\" (\"vc_id\",\"vc_detail\",\"marker_id\",\"approve_id\",\"receipt_id\",\"cash_amt\",\"chq_acc_no\",\"chque_no\",\"do_date\",\"job_id\",\"vc_type\",\"autoid_abh\",\"compID\",\"comBranch\") values ('$vp_id','$detail','$user_id',DEFAULT,DEFAULT,'$cash_amt',DEFAULT,DEFAULT,'$now_date','$rs_jobid','P',DEFAULT,'TAL','BK01')";
    ...
    $insert_voucher="insert into account.\"voucher\" (\"vc_id\",\"vc_detail\",\"marker_id\",\"approve_id\",\"receipt_id\",\"cash_amt\",\"chq_acc_no\",\"chque_no\",\"do_date\",\"job_id\",\"vc_type\",\"autoid_abh\",\"compID\",\"comBranch\") values ('$vp_id','$detail','$user_id',DEFAULT,DEFAULT,DEFAULT,'$cq_acid','$cq_id','$now_date','$rs_jobid','P',DEFAULT,'TAL','BK01')";
    $rs_voucher=pg_query($insert_voucher);
    if(!$rs_voucher){
        $status++;
    }
...
if($status == 0){
	//ACTIONLOG
		$sqlaction = pg_query("INSERT INTO action_log(id_user, action_desc, action_time) VALUES ('$user_id', '(TAL) บันทึก Voucher Pay', '$datelog')");
	//ACTIONLOG---
    pg_query("COMMIT");
    //pg_query("ROLLBACK");
