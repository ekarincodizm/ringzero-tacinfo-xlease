...
$text_money = pg_escape_string($_POST['text_money']);
$text_drcr = pg_escape_string($_POST['text_drcr']);
$text_accno = pg_escape_string($_POST['text_accno']);
...
if($c_dr<1 or $c_cr<1){
    ...
    foreach($text_money as $key_money2 => $value_money2){
        ...
        if($text_drcr[$key_money2] == 1){
            $in_sql="insert into account.\"AccountBookDetail\" (\"autoid_abh\",\"AcID\",\"AmtDr\",\"AmtCr\",\"RefID\") values ('$res_auto_id','$text_accno[$key_money2]','$value_money2','0','');";
            if($result=pg_query($in_sql)){
                $status = 0;
            }else{
                $status = 1;
            ...
            $in_sql="insert into account.\"AccountBookDetail\" (\"autoid_abh\",\"AcID\",\"AmtDr\",\"AmtCr\",\"RefID\") values ('$res_auto_id','$text_accno[$key_money2]','0','$value_money2','');";
            if($result=pg_query($in_sql)){
                $status = 0;
            }else{
                $status = 1;
