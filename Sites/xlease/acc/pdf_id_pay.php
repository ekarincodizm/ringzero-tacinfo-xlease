...
$p_aid=pg_escape_string($_GET["aid"]);
...
$qry_m=pg_query("select * from account.\"AccountBookHead\" where acb_id='$p_aid' ");
  while($res_m=pg_fetch_array($qry_m))
  {
	...
	$pdf->Text(155,26.0+$arow,$icon_daterec.$a_id); //เลขที่ใบเสร็จ
     ...
     $crow=40;
	 $qry_vacc=pg_query("select *  from account.\"VAccountBook\" where acb_id='$a_id' ");
	 ...
	 if($crow >= 75){
	   ...
	   $pdf->SetXY(15,$crow);//Detail
	   $i_dt=iconv('UTF-8','windows-874',"  ".$sp_dtl);
	   $pdf->MultiCell(170,10,$i_dt,1,'L',0);
