...
<div style="float:left"><input name="button" type="button" onclick="window.location='voucher_payment.php'" value="ย้อนกลับ" /></div>
...
//Insert BookHead
$result=pg_query("insert into account.\"AccountBookHead\" (\"type_acb\",\"acb_id\",\"acb_date\",\"acb_detail\",\"ref_id\") values ('GJ','$res_gj_id','$now_date','$insert_detail','$vp_id');");
if(!$result){
    $status += 1;
}
...
if($type1 == 1 && $type2 == ""){
    $result=pg_query("insert into account.\"AccountBookDetail\" (\"autoid_abh\",\"AcID\",\"AmtDr\",\"AmtCr\",\"RefID\") values ('$res_auto_id','$acid','$sum_amt_all','0','$vp_id');");
    if(!$result){
        $status += 1;
    }
    $result=pg_query("insert into account.\"AccountBookDetail\" (\"autoid_abh\",\"AcID\",\"AmtDr\",\"AmtCr\",\"RefID\") values ('$res_auto_id','$cash_type','0','$cash_amt','$vp_id');");
    if(!$result){
        $status += 1;
    }
}elseif($type2 == 1 && $type1 == ""){
    ...
    $result=pg_query("insert into account.\"AccountBookDetail\" (\"autoid_abh\",\"AcID\",\"AmtDr\",\"AmtCr\",\"RefID\") values ('$res_auto_id','$cq_type','0','$cq_amt','$vp_id');");
    if(!$result){
        $status += 1;
    }
}elseif($type2 == 1 && $type1 == 1){
