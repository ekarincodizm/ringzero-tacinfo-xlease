...
<div id="login"  style="height:50px; width:800px; text-align:left; margin-left:auto; margin-right:auto;">
  ...
  $qry_trpay=pg_query("select * from \"TranPay\" 
                       WHERE (tr_date='$dateqer') AND (tran_type!='TR') AND (branch_id = '$type_branch')");
  $nurow=pg_num_rows($qry_trpay);					   
  ?>
  <table width="776" border="0" cellspacing="1" style="background-color:#999999;">

  <tr style="background-color:#FFFFCC;">
    <td colspan="10">วันที่ <?php echo  $_POST["qryDate"]; if($type_branch == 1){ echo " (สาขานวมินทร์)";}else{ echo " (สาขาจรัญ/ติวานนท์)";}?> </td>
    </tr>
  <?php 
    if($numrow > 0)
	  {
   ...
   */
   ...
   $postLog=$res_tr["PostID"];
	   if($type_branch==1){
		...
		$bt_post="<button onclick=\"window.location='nw_tr_post.php?r1=$ref1&r2=$ref2&trd=$dateqer&plog=$postLog'\">ทำรายการ</button>";
	   }
   }
   else
   {
  ...
  <tr style="background-color:#FFFFFF;">
    <td><?php echo $n; ?></td>
    <td style="padding:3px;" align="center"><?php if($idno ==""){ echo "<font size=2 color=red>ไม่พบข้อมูล</font>";}else{?><a href="../post/frm_viewcuspayment.php?idno_names=<?php echo $idno; ?>&type=outstanding" target="_blank" title="<?php echo $id_regis; ?>"><?php echo $idno; ?></a><?php } ?></td>
    ...
    <td align="center"><?php echo $bt_post; ?></td>
  </tr>
  <?php
  }
  ?>
