...
<div id="login"  style="height:50px; width:800px; text-align:left; margin-left:auto; margin-right:auto;">
  ...
  $qry_trpay=pg_query("select * from \"TranPay\" 
                       WHERE  (post_to_idno is NULL) OR (post_to_idno='')  AND(post_on_asa_sys=FALSE)");
  ...
  <tr style="background-color:#FBFBFB;">
    <td colspan="9">รายการ Bill payment ที่ยังไม่สมบูรณ์</td>
    </tr>
  <?php 
    if($numrow > 0)
	  {
  ...
  <tr style="background-color:#FCF1C5">
    <td colspan="9">ไม่พบข้อมูลการโอนเงิน</td>
    </tr>
  <tr>
  <?php 	   
	  }
  ...
  <tr style="background-color:#DDE6B7">
    ...
    <td width="70"><div align="center">post</div></td>
  </tr>
  <?php
  $qry_tr=pg_query("select * from \"TranPay\" 
                       WHERE (tran_type!='TR')AND (post_to_idno='') AND (post_on_asa_sys=FALSE) ORDER BY tr_date desc");
