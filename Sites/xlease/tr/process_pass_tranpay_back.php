...
<div id="login"  style="height:50px; width:800px; text-align:left; margin-left:auto; margin-right:auto;">
  ...
  <?php
	...
	if($numchk>0){ //ถ้าพบแสดงว่าได้ทำรายการนี้ไปก่อนหน้านี้แล้ว
		...
		$qry_pid=pg_query("select * from \"TranPay\" WHERE \"PostID\"='$post_id' ");
		$res_ppd=pg_fetch_array($qry_pid);
  
		$idno_s=$res_ppd["post_to_idno"];
		$amt_s=$res_ppd["amt"];
 
		 //$idno_id=$_GET["idno"];
		 //$amt_id=$_GET["samt"];
 
		$qry_passtr=pg_query("select pass_tranpay('$post_id','$idno_s','$id_user')");
		...
		<tr style="background-color:#FFFFFF;">
			...
			<td width="398"><?php echo $idno_s; ?></td>
			<td width="145"><?php echo number_format($amt_s,2); ?></td>
