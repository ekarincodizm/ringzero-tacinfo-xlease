...
<div id="login"  style="height:50px; width:800px; text-align:left; margin-left:auto; margin-right:auto;">
  ...
  <br />
  รายงาน Bill payment / เงินโอน
  <br />
  <br />
  <form name="frm_ch" method="post" action="report_transpaydate.php">
  เลือกวันที่ <input name="qryDate" type="text" readonly="true" value="<?php echo date("Y/m/d"); ?>"/>
	  ...
	  <select name="branch">
		<option value="1" selected>สาขานวมินทร์</option>
		<option value="2">สาขาจรัญ/ติวานนท์</option>
	  </select>
	  
	  <select name="s_bank">
	  <option value="0">เลือกทุกธนาคาร</option>
	  <?php
	  $sql_s=pg_query("select bankname,bankno from bankofcompany");
	  while($res_s=pg_fetch_array($sql_s))
	  {
	  ?>
	  <option value="<?php echo $res_s["bankno"]; ?>"><?php echo $res_s["bankname"]; ?></option>
	  <?php
	  }
	  ?>
	  </select>
