   ...
   <script type="text/javascript">
	...
	function createXMLHttpRequest() {
		...
		function startRequest_nuLock() {
			...
			var sNumber = document.getElementById("f_carnum").value;
			var cusNumber = document.getElementById("var_cusid").value;
            xmlHttp.open("get", "lock_idno.php?stalock=1&idnoget=" + sText + "&fcusnum="+ cusNumber +"&fcarnum="+sNumber, true);
            xmlHttp.onreadystatechange = function () {
                if (xmlHttp.readyState == 4) {
                    if (xmlHttp.status == 200) {
                        displayInfo_lock(xmlHttp.responseText);
...
$sidno=trim(pg_escape_string($_POST["idno_names_m"]));
$idno=substr($sidno,0,11);	
  ...
  <tr>
    <td>&nbsp;</td>
    <td colspan="2"><input type="button" value="BACK" onclick="window.location='confrim_fp.php' "/></td>
    <td>&nbsp;</td>
  </tr>
</table>  
