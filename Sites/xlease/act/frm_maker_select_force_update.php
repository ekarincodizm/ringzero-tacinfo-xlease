...
$qry_pid=pg_query("select \"PayID\" from \"insure\".\"PayToInsure\" WHERE \"Company\"='$comp' AND \"kind\"='1' AND \"idauthority\" is null ");
if($res_pid=pg_fetch_array($qry_pid)){
    ...
    $in_sql="insert into \"insure\".\"PayToInsure\" (\"PayID\",\"idmaker\",\"DoDate\",\"kind\",\"Company\") values  ('$res_oins','$get_id_user','$nowdate','1','$comp')";
...
for($i=0;$i<count($cid);$i++){
    ...
    $qry_name=pg_query("select \"Premium\",\"NetPremium\",\"Company\" from \"insure\".\"InsureForce\" WHERE \"InsFIDNO\"='$cid[$i]' ");
    ...
    $c_com=pg_query("select \"insure\".cal_comm('PRB','$Company','$NetPremium')");
    $res_comms=pg_fetch_result($c_com,0);
    $showcom = $Premium - $res_comms;
    
    $in_sql="UPDATE \"insure\".\"InsureForce\" SET \"CoPayInsAmt\"='$showcom',\"Commision\"='$res_comms',\"CoPayInsID\"='$res_oins' WHERE \"InsFIDNO\"='$cid[$i]' ";
    if(!$result=pg_query($in_sql)){
        $status++;
    }
}
...
<br><br>
<input type="button" value="  Back  " onclick="location.href='frm_maker_select_force.php'">
