...
<fieldset><legend><B>อนุมัติ > ประกันภัยภาคสมัครใจ</B></legend>

<div align="right">
<form name="frm_fuc1" method="post" action="frm_approval_select_unforce.php">
เลือกบริษัทประกัน 
...
$qry_dc=pg_query("select DISTINCT A.*,B.* from \"insure\".\"InsureUnforce\" A 
    ...
    while($res_if=pg_fetch_array($qry_dc)){
        ...
        $qry_name=pg_query("select * from insure.\"VInsUnforceDetail\" WHERE \"InsUFIDNO\"='$InsUFIDNO'");
        ...
        $c_com=pg_query("select \"insure\".cal_comm('$Kind','$sCompany','$NetPremium')");
...
<form name="approval" method="post" action="frm_approval_select_unforce_update.php">
...
?>      
    ...
    <td></td>
    <td>จำนวนเงิน</td>
    <td><input type="text" name="cheque_money" id="cheque_money" disabled="true" value="<?php echo $summary; ?>" style="text-align:right;"> บาท.</td>
    </tr>
    <tr align="left">
    <td></td>
    <td>หมายเหตุ</td>
...
<input type="hidden" name="hKind" value="<?php echo $Kind; ?>">
<input type="hidden" name="hCompany" value="<?php echo $sCompany; ?>">
