...
//ACTIONLOG
		$sqlaction = pg_query("INSERT INTO action_log(id_user, action_desc, action_time) VALUES ('$get_userid', '(TAL) เตือนหมดอายุ ประกันสมัครใจ', '$datelog')");
//ACTIONLOG---
...
for($i=0;$i<$c_cid;$i++){
    ...
    $qry_if=pg_query("select * from \"insure\".\"InsureUnforce\" WHERE \"InsUFIDNO\"='$cid[$i]' ");
...
จึงใคร่ขอให้ท่านติดต่อกลับ เกี่ยวกับเรื่องประกันภัยรถยนต์ก่อนวันที่ $ExpireDate นี้ เพราะต้องเตรียมเอกสารยื่นที่
