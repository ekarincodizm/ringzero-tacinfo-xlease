...
$gmm=iconv('UTF-8','windows-874',"ประกันภัยภาคสมัครใจ - ประจำวันที่ ".$restrn);
$pdf->Text(6,26,$gmm);
...
while($res_inf=pg_fetch_array($qry_inf)){
    ...
    $summary = 0;
    $qry_if=pg_query("select \"Company\", \"IDNO\", \"StartDate\", \"InsUFIDNO\", \"InsID\", \"Premium\", \"EndDate\" from \"insure\".\"InsureUnforce\" WHERE \"Company\" = '$company' AND \"ConfirmDate\" = '$gdate' AND \"Cancel\"='FALSE' ORDER BY \"InsID\" ASC");
...
$pdf->MultiCell(196,4,$buss_name,0,'C',0);
    ...
    $pdf->MultiCell(85,4,$s_down,0,'R',0);
