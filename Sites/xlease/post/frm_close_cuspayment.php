...
//ACTIONLOG
		$sqlaction = pg_query("INSERT INTO action_log(id_user, action_desc, action_time) VALUES ('$user_id', '(TAL) คิดยอดปิดบัญชี', '$add_date')");
//ACTIONLOG---

if(strstr($_SERVER['HTTP_USER_AGENT'],'iPhone') || strstr($_SERVER['HTTP_USER_AGENT'],'iPod') || strstr($_SERVER['HTTP_USER_AGENT'],'iPad')){
    header('Location: mobile_frm_close_cuspayment.php');
    exit();
}
...
<form method="post" action="frm_close_cuspayment.php?menu=<?php echo $menu; ?>" name="f_list" id="f_list">
<div>
...
?>
    <tr bgcolor="#FFC0C0" align="left">
        <td>ประกันภัยคุ้มครองหนี้</td><td colspan="3"><?php echo number_format($sum_outstanding3,2); ?></td>
    </tr>
<?php
    }
?>
...
for($b=0; $b<$count_idno; $b++){ // วนลูป IDNO ทั้งหมด
	...
	from \"VCusPayment\" WHERE (\"IDNO\"='$list_idno[$b]') AND (\"R_Date\" is not null)");
...
$qry_before=pg_query("select \"DueNo\",\"DueDate\",\"R_Date\",\"daydelay\",\"CalAmtDelay\",\"R_Receipt\",\"R_Bank\",\"PayType\",\"V_Receipt\",\"V_Date\",
\"R_Money\",\"VatValue\"
 from \"VCusPayment\" WHERE (\"IDNO\"='$list_idno[$b]') AND (\"R_Date\" is not null)");
while($resbf=pg_fetch_array($qry_before)){
?>  
...
$qry_amt=@pg_query("select \"DueNo\",\"DueDate\",\"R_Receipt\",\"R_Bank\",\"PayType\",\"V_Receipt\",\"V_Date\",'$ssdate'- \"DueDate\" AS \"dateA\"  from  \"VCusPayment\" WHERE  (\"IDNO\"='$list_idno[$b]')  AND (\"DueDate\" BETWEEN '$DateUpdate' AND '$ldate') "); 
while($res_amt=@pg_fetch_array($qry_amt)){
