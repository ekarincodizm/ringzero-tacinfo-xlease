...
$fp_fc_gas = checknull($_POST["gas_system"]); //ระบบแก๊ส


$g_type=$_POST["g_type"];//ระบบ gas  LPG NGV


$g_tanksn=checknull($_POST["g_tanknumber"]);//
$g_band=checknull($_POST["g_name"]);//ยี่ห้อ gas
$g_signdate=$_POST["signDate"];//
...
$g_vat_down=pg_query("select amt_before_vat($g_downprice);");
$res_gvat_down=pg_fetch_result($g_vat_down,0);
$g_vatdown=$g_downprice-$res_gvat_down;

$g_vat_month=pg_query("select amt_before_vat($g_month);");
$res_gvat_month=pg_fetch_result($g_vat_month,0);
$g_vatmonth=$g_month-$res_gvat_month;
...
if($ss_carid==1)
{
 ...
 $pfinal_carid=trim($_POST["f_search_fc"]);
	...
	$fp_fc_gas = checknull($res_fc["fc_gas"]); //ระบบแก๊ส
}
else 
{
	/*
...
$gencode="select generate_id('$g_signdate',$officeid,$as_type_gas);";
...
$in_sql="insert into \"Fp\" (\"IDNO\",\"P_STDATE\",\"P_DOWN\",\"P_TOTAL\",\"P_MONTH\",\"P_FDATE\",\"P_BEGIN\",\"P_BEGINX\",\"P_VatOfDown\",\"P_VAT\",asset_type,asset_id,\"CusID\",\"P_CustByYear\") 
values ('$residno','$g_signdate','$res_gvat_down','$g_total','$res_gvat_month','$g_fdate','$g_begin','$g_beginx','$g_vatdown','$g_vatmonth','2','$pre_idsn','$pfinal_cusid','$pcusbyyear');";
if($result=pg_query($in_sql)){
    //echo "OK $in_sql<br>";
    $process_status = 0;
}else{
...
values ('$pre_idsn',$g_band,$g_tanksn,'$g_type',$g_carregis,'$g_pro','$g_year','$g_carnumber','$g_carengine',$C_Milage, $fp_fc_type,$fp_fc_brand ,$fp_fc_model ,$fp_fc_category ,$fp_fc_newcar,$fp_fc_gas );";
...
$gen_cus="select \"CreateCusPayment\"('$residno');";
$res_cus=pg_query($db_connect,$gen_cus);
...
if($process_status==0){
    pg_query("COMMIT");
    echo "บันทึก $residno เรียบร้อยแล้ว";
}else{
    pg_query("ROLLBACK");
