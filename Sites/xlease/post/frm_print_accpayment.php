...
class PDF extends ThaiPDF
{
    function Header() {
        ...
        $this->MultiCell(274,4,$buss_name,0,'R',0);
    }
}
...
if(empty($mode)){
    $qry_idno=pg_query("select \"IDNO\" from \"Fp\" where \"P_CustByYear\" = '$year' AND \"P_TOTAL\" <> '0' ORDER BY \"IDNO\"");
}else{
    $qry_idno=pg_query("select \"IDNO\" from \"Fp\" where \"IDNO\"='$idno_names'");
}
...
if( $res_fp=pg_fetch_array($qry_fp) ){
    ...
    $P_CustByYear=$res_fp["P_CustByYear"];
}
    
if($mode == 1){
    $show_year = $P_CustByYear+543;
}
...
$pdf->SetXY(10,10);
$title=iconv('UTF-8','windows-874',"ตารางชำระเงิน ทางบัญชี ปี $show_year");
...
$pdf->SetFont('AngsanaNew','',12);
    ...
    if( $res_name=pg_fetch_array($qry_name) ){
        $full_name_old=$res_name["full_name"];
...
$pdf->SetXY(5,23);
$buss_name=iconv('UTF-8','windows-874',"เลขที่สัญญา ".$idno_lob."     ชื่อผู้เช่าซื้อ ".$full_name_old);
...
$pdf->SetXY(265,35); 
$buss_name=iconv('UTF-8','windows-874',"ดอกผลค้างรับ");
$pdf->MultiCell(25,4,$buss_name,0,'C',0);                   
...
$pdf->SetXY(5,28);
$buss_name=iconv('UTF-8','windows-874',"ค่างวด ".number_format($fp_pmonth,2)."     ภาษีมูลค่าเพิ่ม ".number_format($fp_pvat,2)."     รวม ".number_format($p_sum,2)."     จำนวนงวด ".$sfp_ptotal);
$pdf->MultiCell(100,4,$buss_name,0,'L',0);
...
$pdf->MultiCell(290,4,$buss_name,0,'C',0);
            ...
            $qry_total=pg_query("select \"P_TOTAL\",\"P_BEGINX\" from \"Fp\" WHERE \"IDNO\"='$sort_idno'");
            $res_total=pg_fetch_array($qry_total); 
            $total_top=$res_total["P_TOTAL"];
            $P_BEGINX=$res_total["P_BEGINX"];
            
            $qry_vcus=pg_query("select * from \"VAccPayment\" WHERE \"R_Date\" is not null and \"IDNO\"='$sort_idno' order by \"DueDate\" ");
            while($res_vcus=pg_fetch_array($qry_vcus)) {
                ...
                if($nub_show == 1){
                    $pdf->SetXY(190,40);
                    $buss_name=iconv('UTF-8','windows-874',number_format($P_BEGINX,2));
                    $pdf->MultiCell(25,4,$buss_name,0,'C',0);
                }
                ...
                $pdf->SetXY(240,$cline); 
                $buss_name=iconv('UTF-8','windows-874',number_format($res_vcus[accint],2));
                $pdf->MultiCell(25,4,$buss_name,0,'C',0);

                if($nub_show == 1){
                    $pdf->SetXY(265,40);
                    $buss_name=iconv('UTF-8','windows-874',number_format(($pm_non_vat-$P_BEGINX),2));
                    $pdf->MultiCell(25,4,$buss_name,0,'C',0);
                }
...
do{
    ...
    $search_under=$res_toplv["P_TransferIDNO"];
            ...
            $qry_total=pg_query("select \"P_TOTAL\",\"P_BEGINX\" from \"Fp\" WHERE \"IDNO\"='$idno'");
            $res_total=pg_fetch_array($qry_total); 
            $total_top=$res_total["P_TOTAL"];
            $P_BEGINX=$res_total["P_BEGINX"];
...
if($count_under_idno == 0){
    $qry_vcus=pg_query("select * from \"VAccPayment\" WHERE \"IDNO\"='$idno' order by \"DueDate\" ");
}else{
    $qry_vcus=pg_query("select * from \"VAccPayment\" WHERE \"R_Date\" is not null and \"IDNO\"='$idno' order by \"DueDate\" ");
}
    $count_aabb = pg_num_rows($qry_vcus);
    while($res_vcus=pg_fetch_array($qry_vcus)){
    $cline+=5;
    $nub_show += 1;
    if($nub_show == 1){
    ...
    $inub_page += 1;
    if($inub_page == 24 AND $nub_show < $count_aabb){
        $pdf->AddPage();
        $cline = 42;
        $inub_page = 0;
        $line_nub3 = 0;
...
if($numberstep == ($count_under_idno+$get_numberstep) ){
    $qry_vcus=pg_query("select * from \"VAccPayment\" WHERE \"IDNO\"='$sort_idno' order by \"DueDate\" ");
}else{     
    $qry_vcus=pg_query("select * from \"VAccPayment\" WHERE \"R_Date\" is not null and \"IDNO\"='$sort_idno' order by \"DueDate\" ");
}
while($res_vcus=pg_fetch_array($qry_vcus)) {
