...
$qry_vcus2=pg_query("select * from \"VCusPayment\" WHERE  \"R_Receipt\" is null AND \"IDNO\"='$idno' AND \"DueNo\"='$st_dueno_last2';");
if($resvc2=pg_fetch_array($qry_vcus2)) {
    $get_DueDate = $resvc2["DueDate"];
}
...
$qry_fpdt=pg_query("select b.\"Detail\",b.\"Amount\",b.\"MainDetail\" from \"NTHead\" a
					left join \"NTDetail\" b on a.\"NTID\" = b.\"NTID\"
					where a.\"IDNO\" = '$idno' and \"CusState\" = '0' AND b.\"MainDetail\"='true' AND b.\"Detail\" not like '%ค่ามิเตอร์%' ORDER BY b.autoid ASC;");
...
$qry_fpdt=pg_query("select b.\"Detail\",b.\"Amount\",b.\"MainDetail\" from \"NTHead\" a
					left join \"NTDetail\" b on a.\"NTID\" = b.\"NTID\"
					where a.\"IDNO\" = '$idno' and \"CusState\" = '0' AND b.\"MainDetail\"='false' AND b.\"Detail\" not like '%ค่ามิเตอร์%'  ORDER BY b.autoid ASC;");
$rows_dt = pg_num_rows($qry_fpdt);
while($res_fpdt=pg_fetch_array($qry_fpdt)){
    $nub_false+=1;
    $Detail = $res_fpdt["Detail"];
...
// -- todo: ต้องแก้อะไรอีกหรือไม่
$title=iconv('UTF-8','windows-874',"          โดยหนังสือฉบับนี้ ข้าพเจ้าในฐานะผู้รับมอบอำนาจจาก บริษัทฯ จึงขอให้ท่านชำระหนี้จำนวนดังกล่าวให้ครบถ้วนทันงวด ภายในกำหนด 30 วัน นับตั้งแต่วันที่ท่านได้รับหนังสือฉบับนี้ หรือถือว่าได้รับหนังสือโดยชอบด้วยกฏหมายหากพ้นกำหนดนี้แล้ว ท่านยังเพิกเฉย หรือชำระเพียงบางส่วน หรือชำระไม่ครบถ้วนทันงวดตามสัญญา หรือไม่ชำระภายในกำหนดดังกล่าวข้างต้น ให้ถือว่าหนังสือฉบับนี้เป็นการแสดงเจตนาบอกเลิกสัญญา ซึ่งมีผลให้สัญญาเช่าซื้อเป็นอันยกเลิกโดยปริยาย ซึ่งจะทำให้ท่านเสียค่าใช้จ่ายเพิ่มขึ้นอีกมากโดยไม่จำเป็น นับถัดจากวันที่ครบกำหนดดังกล่าวข้างต้น โดยไม่ต้องบอกกล่าวอีก");
}
$pdf->MultiCell(185,6,$title,0,'L',0);
...
//================================= New Page ======================================//
$sum_amt_alls=0;
$qry_cc=pg_query("select * from \"ContactCus\" WHERE \"IDNO\"='$idno' AND \"CusState\" > 0 ORDER BY \"CusState\" ASC;");
while($res_cc=pg_fetch_array($qry_cc)){
    $get_CusID = trim($res_cc["CusID"]);
    $get_CusState = trim($res_cc["CusState"]);
