...
from carregis.\"CarTaxDue\" A LEFT OUTER JOIN carregis.\"DetailCarTax\" B on A.\"IDCarTax\" = B.\"IDCarTax\" 
    where \"CoPayDate\" = '$d' AND \"TypePay\" <> '-1' ORDER BY \"IDNO\" ASC ");
...
$pdf->SetXY(4,$cline-3); 
$buss_name=iconv('UTF-8','windows-874',"______________________________________________________________________________________________________________________________________________________________________");
$pdf->MultiCell(196,4,$buss_name,0,'C',0);
...
$pdf->SetFont('AngsanaNew','',10);
$pdf->SetXY(120,$cline);
$buss_name=iconv('UTF-8','windows-874',"รวมเงิน ".number_format($sum_TaxValue2,2));
$pdf->MultiCell(80,4,$buss_name,0,'R',0);
...
$buss_name=iconv('UTF-8','windows-874',"รวมยอดเงินทั้งสิ้น ".number_format($sum_TaxValue,2));
$pdf->MultiCell(80,4,$buss_name,0,'R',0);
