...
if( isset($mm) and isset($yy) ){
        ...
        $qry_name=pg_query("SELECT * FROM carregis.\"CarTaxDue\" where EXTRACT(MONTH FROM \"TaxDueDate\")='$mm' AND EXTRACT(YEAR FROM \"TaxDueDate\")='$yy' AND (\"TypeDep\"='101' OR \"TypeDep\"='105') ORDER BY \"IDNO\" ASC ");
...
?>
        ...
        <td align="center">
            <?php if($ApointmentDate == "-"){ ?>
                -
            <?php }else{ ?>
                <a href="frm_car_admin_edit.php?cid=<?php echo "$IDCarTax";?>">แก้ไข</a>
            <?php } ?>
        </td>
        <td>
        <?php if($BookIn=='f'){ ?>
            <a href="#" onclick="javascript:confirm1('frm_car_admin_del.php?cid=<?php echo "$IDCarTax";?>');" >ลบ</a></td>
        <?php }else{ ?>
            R
        <?php } ?>
    </tr>
