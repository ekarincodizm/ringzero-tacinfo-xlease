...
$qry_name2=pg_query("select  \"asset_id\",\"full_name\",\"IDNO\",\"CusID\",\"asset_type\",\"C_REGIS\",\"car_regis\",\"C_StartDate\" from \"VContact\" WHERE \"IDNO\"='$id' ");
...
<?php
    ...
    $qry_if=pg_query("select A.\"IDCarTax\",A.\"TaxDueDate\",A.\"TypeDep\",B.\"TaxValue\",A.\"ApointmentDate\",B.\"BillNumber\",B.\"TypePay\"  from carregis.\"CarTaxDue\" A INNER JOIN carregis.\"DetailCarTax\" B on A.\"IDCarTax\" = B.\"IDCarTax\" 
    WHERE \"IDNO\"='$id' ORDER BY A.\"IDCarTax\" ASC");
    $rows = pg_num_rows($qry_if);
    while($res_if=pg_fetch_array($qry_if)){
        $IDCarTax = $res_if["IDCarTax"];
        $TaxDueDate = $res_if["TaxDueDate"];
        $TypeDep= $res_if["TypeDep"];
        ...
        $BillNumber = $res_if["BillNumber"];
        $TypePay = $res_if["TypePay"];
        
        $qry_name8=pg_query("select * from \"TypePay\" WHERE \"TypeID\"='$TypePay' ");
...
?>
        ...
        <td align="right"><?php echo "$BillNumber"; ?></td>
