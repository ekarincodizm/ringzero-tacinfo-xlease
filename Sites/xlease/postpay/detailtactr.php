...
<div style="float:left"><input type="button" value=" Back " class="ui-button" onclick="window.location='selecttactr.php';"></div>
...
<table width="100%" cellpadding="3" cellspacing="0" border="0" style="background-color:#FFD5D5; border-style: dashed; border-width: 1px; border-color:#969696; margin-bottom:3px">
	<tr style="font-weight:bold">
		<td>เลขที่บัญชีธนาคารที่นำเงินเข้า</td>
		<td>ธนาคาร</td>
		<td>วันที่โอนเงินเข้าธนาคาร</td>
		<td>เวลาที่โอน</td>
	</tr>
	<tr bgcolor="#FFECEC">
		<td width="25%"><input type="text" name="chknum" id="chknum"></td>
		<td width="25%"><input type="text" name="chkbank" id="chkbank" size="25"></td>
		<td>
			<input type="text" id="chkdate" name="chkdate" id="chkdate" value="<?php echo $startDate;?>" size="15" style="text-align: center;">
		</td>
		<td>
			<select name="t_hr" id="t_hr">
...
$(document).ready(function(){
    ...
    + '<option value="">เลือก</option>'
    + ' <?php
    $qry_fp=pg_query("select \"TypeID\", \"TName\" from \"TypePay\" WHERE \"TypeID\" <> '299' ORDER BY \"TypeID\" ASC");
    while($res_fp=pg_fetch_array($qry_fp)){
        $TypeID=$res_fp["TypeID"];
        $TName=$res_fp["TName"];
        echo "<option value=$TypeID>$TName</option>";
    ...
    $("#submitButton").click(function(){
        ...
        if( $('#chknum').val() == "" ){
				...
				alert('กรุณาระบุชั่วโมงที่โอนเงิน !');
                $("#submitButton").attr('disabled', false);
                return false;
		}else if($('#t_m').val() == ""){
				alert('กรุณาระบุนาทีที่โอนเงิน !');
                $("#submitButton").attr('disabled', false);
                return false;
		}else if($('#t_s').val() == ""){
				alert('กรุณาระบุวินาทีที่โอนเงิน !');
                $("#submitButton").attr('disabled', false);
                return false;
		}
