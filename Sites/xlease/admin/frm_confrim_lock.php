...
$statuslock = pg_escape_string($_GET['statuslock']);
if($statuslock == "Lock"){
	$txt_lock = "Lock Contact";
}else{
	$txt_lock = "ปลด Lock Contact";
}
?>
...
<title>อนุมัติสัญญาเช่าซื้อ</title>
	...
	function createXMLHttpRequest() {
		...
		function startRequest_Lock() {
            ...
            xmlHttp.open("get", "lock_idno.php?stalock=0&idnoget=" + sText + "&fcusnum="+ cusNumber +"&fcarnum="+sNumber, true);
		...
		function startRequest_nuLock() {
            ...
            xmlHttp.open("get", "lock_idno.php?stalock=1&idnoget=" + sText + "&fcusnum="+ cusNumber +"&fcarnum="+sNumber+"&fass_id="+as_id, true);
            xmlHttp.onreadystatechange = function () {
                if (xmlHttp.readyState == 4) {
                    if (xmlHttp.status == 200) {
                        displayInfo_lock(xmlHttp.responseText);
...
<div class="style1" id="menu" style="height:25px; padding-left:10px; padding-top:10px; padding-right:10px;">อนุมัติสัญญาเช่าซื้อ  (<?php echo $txt_lock;?>)<hr /></div>
...
//$idno=substr($sidno,0,11);	
   ...
   */
   ...
   if($reslock=='t')
   {
	...
	}else if($statuslock == 'Lock'){
		$strlock="Lock แล้ว <br><font color=red>ไม่สามารถ Lock ได้อีก</font>";
   ...
   else
   {
	...
	}else if($statuslock == 'Unlock'){
		$strlock="ยังไม่ได้ Lock <br><font color=red>ไม่สามารถปลด Lock ได้</font>";
		$bt_lock="<input type=\"button\" value=\"ปลด Lock Contact\" name=\"lockidno\" id=\"lockidno\" disabled/>";
	}
   }
   /*
   if($res_fp["P_BEGIN"]!=0)
   {
    $bt_ccc="บันทึก Cuspayment:P_BEGIN=$res_fp[P_BEGIN]";
	 $bt_ccc2="<input name=\"button2\" type=\"button\" value=\"Create Customer Payment\" onclick=\"startRequest_ccc()\" />";
   }
   else
   {
   ...
   if($res_fp["P_BEGINX"]!=0)
   {
    $bt_acc="บันทึก Accpayment:P_BEGINX=$res_fp[P_BEGINX]";
	 $bt_acc2="<input type=\"button\" value=\" Create Account Payment \" onclick=\"startRequest_acc()\"/>";
   }
   else
   {
