...
$nowdate = date("d/m/Y");
$f_idno = pg_escape_string($_REQUEST["h_id"]);

$qry_chk_gas = pg_query("select \"IDNO\" FROM \"Fp\" WHERE \"IDNO\" = '$f_idno' AND \"asset_id\" IN (select \"CarID\" FROM \"Fc\") ");
$row_chk_gas = pg_num_rows($qry_chk_gas);
if($row_chk_gas == 0){
	echo "<meta http-equiv=\"refresh\" content=\"0; URL=frm_rpt_ck_gas.php?h_id=$f_idno\">";
	exit();
}
...
A.\"P_CLDATE\", A.\"P_ACCLOSE\",B.\"A_FIRNAME\",B.\"A_NAME\",B.\"A_SIRNAME\",B.\"A_PAIR\",C. \"CarID\" ,C.\"C_CARNAME\", 
C.\"C_YEAR\", C.\"C_REGIS\",C.\"C_REGIS_BY\", C.\"C_COLOR\", C.\"C_CARNUM\",C.\"C_Milage\", C.\"C_MARNUM\",
to_char(C.\"C_TAX_ExpDate\",'dd/mm/yyyy') AS c_tax_exp, C.\"C_TAX_MON\",D.\"A_NO\",D.\"A_SUBNO\",D.\"A_SOI\",
D.\"A_RD\",D.\"A_TUM\",D.\"A_AUM\",D.\"A_PRO\",D.\"A_POST\",C.\"fc_type\", C.\"fc_brand\", C.\"fc_model\", C.\"fc_category\", C.\"fc_newcar\",C.\"fc_gas\"
FROM \"Fp\" A  
...
$qry_fn=pg_query("select \"N_SAN\",\"N_AGE\",\"N_CARD\",\"N_IDCARD\",\"N_CARDREF\",\"N_OCC\",\"N_BY\",to_char(\"N_OT_DATE\",'dd/mm/yyyy') AS otdate from \"Fn\" where \"CusID\"='$cus_id'  ");
...
$fc_gas = $res_p["fc_gas"]; //ระบบแก๊สรถยนต์	
...
$car_color_icon=iconv('UTF-8','windows-874',"สีรถ : ".$car_color);
...
$car_number_icon=iconv('UTF-8','windows-874',"ระบบแก๊สรถยนต์ : ".$fc_gas );
$pdf->Text(50+$w3,93+$arow,$car_number_icon); //ระบบแก๊สรถยนต์

$car_engine_icon=iconv('UTF-8','windows-874',"เลขเครื่อง : ".$car_engine);
$pdf->Text(50+$w3+$w4,93+$arow,$car_engine_icon); //เลขเครื่อง

$car_mi_icon=iconv('UTF-8','windows-874',"เลขกิโล : ".$car_mi);
$pdf->Text(10,100+$arow,$car_mi_icon); //เลขกิโล

$car_engine_icon=iconv('UTF-8','windows-874',"วันที่ภาษีป้ายหมดอายุ : ".$car_tax_exp);
$pdf->Text(70,100+$arow,$car_engine_icon); 

$car_engine_icon=iconv('UTF-8','windows-874',"ค่าภาษีป้าย : ".$car_tax_amount);
$pdf->Text(50+$w3+$w4,100+$arow,$car_engine_icon); 

$car_year_icon=iconv('UTF-8','windows-874',"ปี : ".$car_year);
$pdf->Text(10,107+$arow,$car_year_icon); //ปี

$car_year_icon=iconv('UTF-8','windows-874',"ชนิดรถ : ".$fp_fc_category);
$pdf->Text(70,107+$arow,$car_year_icon); //ชนิดรถ

$car_regis_icon=iconv('UTF-8','windows-874',"เป็นรถ : ".$newcar);
$pdf->Text(50+$w3,107+$arow,$car_regis_icon); //ทะเบียน

$car_province_icon=iconv('UTF-8','windows-874',"ประเภทรถ : ".$fp_type);
$pdf->Text(50+$w3+$w4,107+$arow,$car_province_icon); //ประเภทรถ
...
//รายละเอียดประกันภัย
$qry_if=pg_query("select \"IDNO\",\"InsID\",\"Invest\",\"Premium\",\"NetPremium\",\"Company\",to_char(\"ConfirmDate\",'dd/mm/yyyy') AS c_d from \"insure\".\"InsureUnforce\" WHERE \"IDNO\" = '$f_idno' AND \"Cancel\"='FALSE' ");
...
//รายละเอียด พรบ.
$qry_if=pg_query("select \"IDNO\",\"InsID\",\"InsMark\",\"Code\",\"Premium\",\"NetPremium\",\"Company\",to_char(\"DoDate\",'dd/mm/yyyy') AS d_d,to_char(\"StartDate\",'dd/mm/yyyy') AS s_d from \"insure\".\"InsureForce\" WHERE \"IDNO\" = '$f_idno' AND \"Cancel\"='FALSE' ");
$rows = pg_num_rows($qry_if);
while($res_if=pg_fetch_array($qry_if)){
   // $InsUFIDNO = $res_if["InsUFIDNO"];
    $IDNO = $res_if["IDNO"];
...
//ผู้ค้ำ
$qry_print_dat=pg_query("select \"CusID\" from \"ContactCus\" where (\"IDNO\" = '$f_idno') AND (\"CusState\"!=0) order by \"CusState\"");						   
