...
//หารายการเดิมของ Head เพื่อเอาไว้ตรวจสอบและเซต cancel เป็น true
$qry_array_chk=pg_query("SELECT \"auto_id\" FROM account.\"AccountBookHead\" WHERE \"ref_id\" LIKE 'BREC%' AND EXTRACT(MONTH FROM \"acb_date\")='$mm' AND EXTRACT(YEAR FROM \"acb_date\")='$yy' AND \"cancel\"='false'");
...
while($res_in=pg_fetch_array($qry_in)){
    ...
    if(@empty($cur_year) AND $R_DueNo < 99){
        $text_error[] = "empty CustYear $R_Date:$R_Receipt<br />";
        $status++;
        break;
    }
    ...
    if($R_Date != $old_date){ //ตรวจสอบเพื่อสรุปยอดรวมแต่ละวัน
        ...
        foreach($show_unique_CustYear as $v){
            ...
            $qry_ac=@pg_query("SELECT \"AcID\" FROM account.\"AcTable\" WHERE \"AcType\"='CUR1'");
            if($res_ac=@pg_fetch_array($qry_ac)){
            ...
            $count_autoid = 0;
            $qry_chk=@pg_query("SELECT COUNT(\"auto_id\") as \"count_autoid\" FROM account.\"AccountBookHead\" WHERE \"acb_id\" LIKE 'AP%' AND \"ref_id\" LIKE 'BREC%' AND \"acb_date\"='$old_date' AND \"acb_detail\" LIKE '%$v' AND \"cancel\"='false' ");
            ...
            if($count_autoid != 0){
                if($count_autoid == 1){
                    ...
                    $qry_chk_w=@pg_query("SELECT \"auto_id\",\"acb_detail\" FROM account.\"AccountBookHead\" WHERE \"acb_id\" LIKE 'AP%' AND \"ref_id\" LIKE 'BREC%' AND \"acb_date\"='$old_date' AND \"acb_detail\" LIKE '%$v' AND \"cancel\"='false' ORDER BY \"auto_id\" ASC");
                    while($res_chk_w=@pg_fetch_array($qry_chk_w)){
                        ...
                        if( strstr($acb_detail,"ภาษี") ){
                            $up_sql="UPDATE \"account\".\"AccountBookHead\" SET \"type_acb\"='AP',\"acb_date\"='$old_date',\"acb_detail\"='บันทึกรับเงินลูกหนี้ภาษีมูลค่าเพิ่มค้างรับของลูกหนี้ปี $v',
                            ...
                            $up_sql="UPDATE \"account\".\"AccountBookHead\" SET \"type_acb\"='AP',\"acb_date\"='$old_date',\"acb_detail\"='บันทึกรับเงินลูกหนี้ปี $v',\"ref_id\"='BREC' 
                            ...
                            $in_sql="insert into \"account\".\"AccountBookHead\" (\"type_acb\",\"acb_id\",\"acb_date\",\"acb_detail\",\"sub_type\",\"ref_id\") 
                            values  ('AP','$genid','$old_date','บันทึกรับเงินลูกหนี้ภาษีมูลค่าเพิ่มค้างรับของลูกหนี้ปี $v',DEFAULT,'BREC')";
                            if(!$res_in_sql=@pg_query($in_sql)){
                                $text_error[] = "INSERT AccountBookHead<br />";
                                $status++;
                                break;
                ...
                if($cuvat != 0 OR $cavat != 0){
                    ...
                    $qry_chk_w=@pg_query("SELECT \"auto_id\" FROM account.\"AccountBookHead\" WHERE \"acb_id\" LIKE 'AP%' AND \"ref_id\" LIKE 'BREC%' AND \"acb_date\"='$old_date' AND \"acb_detail\" LIKE '%$v' AND \"cancel\"='false' ORDER BY \"auto_id\" ASC");
                    while($res_chk_w=@pg_fetch_array($qry_chk_w)){
                        $auto_id = $res_chk_w["auto_id"];
                        $diff = array($auto_id);
                        $arr_chk = array_diff($arr_chk, $diff);
            ...
            }else{  //ข้อมูลไม่ซ้ำ ให้ทำการ Insert ใหม่
