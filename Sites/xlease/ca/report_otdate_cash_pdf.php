...
$pdf->SetXY(10,10);
$title=iconv('UTF-8','windows-874',"รายงาน รับเงินสดจ่ายค่าอื่นๆ");
...
$i = 1;
$j = 0;
$qry_ot=pg_query("select * from \"VFOtherpayEachDay\" WHERE (\"O_PRNDATE\"='$tday') AND (\"O_BANK\"='CA') AND \"PayType\" = 'OC' ORDER BY \"O_RECEIPT\" ");                           
while($res_if=pg_fetch_array($qry_ot)){
    $j+=1;
    $aa+=1;
...
$qry_ot=pg_query("select * from \"VFOtherpayEachDay\" WHERE (\"O_PRNDATE\"='$tday')  AND (\"PayType\" is null) ORDER BY \"O_BANK\" ASC,\"O_RECEIPT\" ASC");                           
while($res_if=pg_fetch_array($qry_ot)){
    $j+=1;
    $aa+=1;
        $R_Receipt = $res_if["O_RECEIPT"];
