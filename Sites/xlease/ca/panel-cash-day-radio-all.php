...
if($num_row>0){
    echo "<tr><td class=\"sum\" align=center><a href=\"cash_day_radio_all_pdf.php?date=$datepicker&type=1\" target=\"_blank\"><span style=\"font-size:15px; color:#0000FF;\">(พิมพ์รายงาน)</span></a></td><td colspan=6 class=\"sum\" align=right><b>รวมเงิน</b></td><td align=right class=\"sum\"><b>".number_format($sum_amt,2)."</b></td><td class=\"sum\"></td></tr>";
}

//=============================================//
$num_row = 0;
$sum_amt = 0;
$query=pg_query("select * from \"FOtherpay\" WHERE \"O_PRNDATE\"='$datepicker' AND \"O_Type\"='109' AND \"PayType\"<>'OC' AND \"Cancel\"='FALSE' ORDER BY \"PayType\",\"O_DATE\" ASC");
$num_row = pg_num_rows($query);
while($resvc=pg_fetch_array($query)){
    $IDNO = $resvc['IDNO'];
    $O_DATE = $resvc['O_DATE'];
...
if($num_row>0){
    echo "<tr><td class=\"sum\" align=center><a href=\"cash_day_radio_all_pdf.php?date=$datepicker&type=2\" target=\"_blank\"><span style=\"font-size:15px; color:#0000FF;\">พิมพ์รายงาน</span></a></td><td colspan=6 class=\"sum\" align=right><b>รวมเงิน</b></td><td align=right class=\"sum\"><b>".number_format($sum_amt,2)."</b></td><td class=\"sum\"></td></tr>";
}
