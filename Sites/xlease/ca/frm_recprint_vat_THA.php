  ...
  $paybyT=substr($res_qry["payby"],22,300);
   ...
   {	
	  ...
	  $r_amt_bf =$money+$vat;
		...
		if($srid=='R'){
			$qry_con=pg_query("select \"R_Receipt\",\"R_Date\",\"Cancel\" from \"Fr\" WHERE \"R_Receipt\" ='$recid'");
...
// start PDF //
  ...
  $pdf->SetFont('AngsanaNew','',15);
	if($Cancel == 't'){
		$pdf->Image("image/cancel.png",60,35,100);  // พิมพ์ยกเลิกกรณีที่เป็นใบเสร็จที่ยกเลิกแล้ว
	}	
  $col=10;
