...
if($type == 1){
    ...
    $pagename = "จากที่อื่น";
    $qry_fr=pg_query("select \"R_Receipt\",\"PayType\",sum(value) as value,sum(vat) as vat,sum(money) as money,sum(discount) as discount from \"VFrEachDay\" WHERE (\"R_Prndate\"='$tday') AND (\"R_memo\"<>'TR-ACC' OR \"R_memo\" is null  OR \"R_memo\"='') GROUP BY \"R_Receipt\",\"PayType\" ORDER BY \"PayType\" ASC ");
}elseif($type == 4){
    $pagename = "เงินโอน";
    $qry_fr1=pg_query("select \"R_Receipt\",\"PayType\",sum(value) as value,sum(vat) as vat,sum(money) as money,sum(discount) from \"VFrEachDay\" WHERE (\"R_Prndate\"='$tday') AND (\"PayType\"<>'OC') GROUP BY \"R_Receipt\",\"PayType\" ORDER BY \"PayType\" ASC ");
...
while($res_if=pg_fetch_array($qry_fr1)){
    ...
    $qry_ss=pg_query("select \"R_Date\", \"R_Bank\", \"PayType\", \"IDNO\", \"full_name\", \"assetname\", \"typepay_name\", \"regis\"
					from \"VFrEachDay\" WHERE (\"R_Receipt\"='$res_if[R_Receipt]') ORDER BY \"typepay_name\" DESC ");
...
$pdf->SetXY(151,$cline); 
$buss_name=iconv('UTF-8','windows-874',"รวมยอดเงิน (ACC) ");
$pdf->MultiCell(50,4,$buss_name,0,'R',0);        
