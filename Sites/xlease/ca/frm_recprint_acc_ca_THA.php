...
$num_row_dch=pg_num_rows($qry_dch);
if($num_row_dch==0){
	$qry_dch=pg_query("select * from \"FTACCheque\" WHERE \"PostID\"='$pid' ");
	$num_dch=pg_num_rows($qry_dch);
	if($num_dch ==0){
		$qry_dch=pg_query("select * from \"FTACTran\" WHERE \"PostID\"='$pid' ");
		$num_dtr=pg_num_rows($qry_dch);
	}
}
  ...
  $num_cuslist=pg_num_rows($cuslist);
	if($num_cuslist==0){
		if($num_dch > 0){
			...
			where  a.\"COID\"='$sidno' and \"refreceipt\"='$recid'");
		}else{
			if($num_dtr==0){
...
if($carregis !=""){
  ...
  $taxdate=$rescus["C_TAX_ExpDate"];
  if($taxdate==""){
	$taxdate=nowDate();//ดึง วันที่จาก server
  }
  $exp_tax=$taxdate;
...
// start PDF //
  ...
  $col=10;
		...
		$ext_join = receipt_d($sidno,$recid) ;
		...
		$av_fullname=iconv('UTF-8','windows-874',$sidno); 
