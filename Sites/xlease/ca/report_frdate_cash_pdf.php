...
$pdf->SetXY(10,10);
$title=iconv('UTF-8','windows-874',"รายงาน รับเงินสดค่างวด");
...
$qry_fr=pg_query("select * from \"VFrEachDay\" WHERE (\"R_Prndate\"='$tday') AND (\"R_Bank\"='CA') AND \"PayType\" = 'OC' ORDER BY \"R_Receipt\"");
while($res_if=pg_fetch_array($qry_fr)){
    $j+=1;
    $aa+=1;
        $R_Receipt = $res_if["R_Receipt"];
        $R_Date = $res_if["R_Date"];
        $R_Bank = $res_if["R_Bank"];
        $IDNO = $res_if["IDNO"];
        $full_name = $res_if["full_name"];
        $assetname = $res_if["assetname"];
        $typepay_name = $res_if["typepay_name"];
        $regis = $res_if["regis"];
        $money = $res_if["money"];
...
$qry_fr=pg_query("select * from \"VFrEachDay\" WHERE (\"R_Prndate\"='$tday') AND (\"PayType\" is null) ORDER BY \"R_Bank\" ASC,\"R_Receipt\" ASC");
while($res_if=pg_fetch_array($qry_fr)){
    $j+=1;
    $aa+=1;
        $R_Receipt = $res_if["R_Receipt"];
