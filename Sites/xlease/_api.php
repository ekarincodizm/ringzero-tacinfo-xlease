<?php
  ...
  else if ($_POST["cmd"] == "list_typepay")
  {
    ...
    $load_typepay = $outlet->from("typepay")->orderBy("\"TypeID\"")->find();
  ...
  else if ($_POST["cmd"] == "depositpay")
  {
    ...
    $payment = json_decode(stripcslashes(pg_escape_string($_POST["payment"]));   
    ...
    foreach ($payment as $p)
    {
      ...
      $get_receipt = $outlet->query("select use_deposit_remain('" . pg_escape_string($_POST['idno']) . "' , " . $p->amount . " , " . $p->typepay . " , '" . $optionStr . "')");
      $row = $get_receipt->fetch();  
      $receiptid = $row[0];
