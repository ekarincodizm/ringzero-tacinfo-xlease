...
$corp_regis_old = $_GET["corp_regis"]; // เลขทะเบียนนิติบุคคล(13 หลัก) ของเก่า
$corp_regis = $_POST["corp_regis"]; // เลขทะเบียนนิติบุคคล(13 หลัก)
$corp_regis_master = $corp_regis; // ใช้เช็คใน sql

if($corp_regis_old != $corp_regis)
{
	$hidden = "yes";
}
...
{
	// ตรวจสอบก่อนว่ามีนิติบุคคลนี้ัอยู่ในระบบหรือยัง เพราะ process นี้จะเป็นการแก้ไขนิติบุคคลที่ยังไม่ได้รับการอนุมัติ เพราะฉนั้นจะต้องยังไม่มีข้อมูลในตารางหลัก
	...
	else
	{
		...
		$Proportion_in_country = $_POST["Proportion_in_country"]; // สัดส่วนการขายในประเทศ
		$Proportion_out_country = $_POST["Proportion_out_country"]; // สัดส่วนการขายต่างประเทศ
		$Proportion_Cash = $_POST["Proportion_Cash"]; // สัดส่วนการขายเงินสด
		$Proportion_Credit = $_POST["Proportion_Credit"]; // สัดส่วนการขายเงินเชื่อ
		$Amount_Employee = $_POST["Amount_Employee"]; // จำนวนพนักงาน
		...
		$rowBank = $_POST["rowBank"]; // จำนวนบัญชีธนาคารที่แท้จริง
		$FullrowBank = $_POST["FullrowBank"]; // จำนวนบัญชีธนาคารที่กดปุ่มเพิ่ม
		//for($b=1;$b<=$rowBank;$b++)
		...
		$rowBoard = $_POST["rowBoard"]; // จำนวนกรรมการที่แท้จริง
		$FullrowBoard = $_POST["FullrowBoard"]; // จำนวนกรรมการที่กดปุ่มเพิ่มทั้งหมด
		//for($c=1;$c<=$rowBoard;$c++)
		for($c=1;$c<=$FullrowBoard;$c++)
		{
			$BoardName[$c] = $_POST["BoardName$c"]; // ชื่อกรรมการ
			$havefileBoard[$c] = $_POST["havefileBoard$c"]; // ลายเซ็นต์เก่าของกรรมการ
			$BoardNameChkNmae[$c] = $_POST["BoardName$c"]; // ชื่อกรรมการ สำหรับเช็คข้อมูลเก่า
			...
			else
			{
				...
				if($BoardName[$c] != "NULL" && $havefileBoard[$c] != "")
				{
					...
					{
						$qry_oldfileBoard  = pg_query("select * from public.\"th_corp_board_temp\" where \"CusID\" = '$havefileBoard[$c]' and \"corp_regis\" = '$corp_regis_master' and \"Approved\" = 'false' and \"boardEdit\" = '$maxedit' ");
		...
		$rowShare = $_POST["rowShare"]; // จำนวนผู้ถือหุ้นที่แท้จริง
		$FullrowShare= $_POST["FullrowShare"]; // จำนวนผู้ถือหุ้นที่กดปุ่มเพิ่มทั้งหมด
		...
		for($d=1;$d<=$FullrowShare;$d++)
		{
			...
			$ShareNameChkNmae[$d] = $_POST["ShareName$d"]; // ชื่อผู้ถือหุ้น สำหรับเช็คข้อมูลเก่า
			
			$havefileShare[$d] = $_POST["havefileShare$d"]; // ลายเซ็นต์เก่าของผู้ถือหุ้น
			...
			else
			{
				...
				if($ShareName[$d] != "NULL" && $havefileShare[$d] != "")
				{
					...
					{
						$qry_oldfileShare = pg_query("select * from public.\"th_corp_share_temp\" where \"CusID\" = '$havefileShare[$d]' and \"corp_regis\" = '$corp_regis_master' and \"Approved\" = 'false' and \"shareEdit\" = '$maxedit' ");
		...
		$rowAttorney = $_POST["rowAttorney"]; // จำนวนกรรมการที่แท้จริง
		$FullrowAttorney = $_POST["FullrowAttorney"]; // จำนวนกรรมการที่กดปุ่ม
		//for($f=1;$f<=$rowAttorney;$f++)
		for($f=1;$f<=$FullrowAttorney;$f++)
		{
			$AttorneyName[$f] = $_POST["AttorneyName$f"]; // ผู้รับมอบ
			$AttorneyNameChkNmae[$f] = $_POST["AttorneyName$f"]; // ผู้รับมอบ สำหรับเช็คข้อมูลเก่า
			$havefileAttorney[$f] = $_POST["havefileAttorney$f"]; // ไฟล์เก่าของผู้รับมอบ
			...
			else
			{
				...
				if($AttorneyName[$f] != "NULL" && $havefileAttorney[$f] != "")
				{
					...
					{
						$qry_oldfileAttorney = pg_query("select * from public.\"th_corp_attorney_temp\" where \"CusID\" = '$havefileAttorney[$f]' and \"corp_regis\" = '$corp_regis_master' and \"Approved\" = 'false' and \"attorneyEdit\" = '$maxedit' ");
		...
		//--- ถ้ามีการเปลี่ยนเลขที่นิติบุคคล จะต้องไปซ้่อนเลขที่นิติบุคคลเก่าด้วย
		if($hidden == "yes")
		{
			$sql_hidden_corp = "update \"th_corp_temp\" set \"hidden\" = 'TRUE' where \"corp_regis\" = '$corp_regis_old' ";
			if($result=pg_query($sql_hidden_corp))
			{}
			else
			{
				$status++;
			...
			$sql_hidden_adds = "update \"th_corp_adds_temp\" set \"hidden\" = 'TRUE' where \"corp_regis\" = '$corp_regis_old' ";
			if($result=pg_query($sql_hidden_adds))
			{}
			else
			{
				$status++;
			...
			$sql_hidden_acc = "update \"th_corp_acc_temp\" set \"hidden\" = 'TRUE' where \"corp_regis\" = '$corp_regis_old' ";
			if($result=pg_query($sql_hidden_acc))
			{}
			else
			{
				$status++;
			...
			$sql_hidden_acc = "update \"th_corp_attorney_temp\" set \"hidden\" = 'TRUE' where \"corp_regis\" = '$corp_regis_old' ";
			if($result=pg_query($sql_hidden_acc))
			{}
			else
			{
			...
			$sql_hidden_acc = "update \"th_corp_board_temp\" set \"hidden\" = 'TRUE' where \"corp_regis\" = '$corp_regis_old' ";
			if($result=pg_query($sql_hidden_acc))
			{}
			else
			{
			...
			$sql_hidden_acc = "update \"th_corp_communicant_temp\" set \"hidden\" = 'TRUE' where \"corp_regis\" = '$corp_regis_old' ";
			if($result=pg_query($sql_hidden_acc))
			{}
			else
			{
			...
			$sql_hidden_acc = "update \"th_corp_share_temp\" set \"hidden\" = 'TRUE' where \"corp_regis\" = '$corp_regis_old' ";
			if($result=pg_query($sql_hidden_acc))
			{}
			else
			{
...
else
{
	...
	//echo "<br>$qry_oldfileBoard";
	//echo "<meta http-equiv='refresh' content='2; URL=frm_IndexAdd.php'>";
	echo "<form method=\"post\" name=\"form2\" action=\"frm_EditCorpAll.php?corp_regis=$corp_regis_old&view=3\">";
