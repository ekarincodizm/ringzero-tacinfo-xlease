...
{
	$query_lastedit = pg_query("select max(\"corpEdit\") as \"maxedit\" from public.\"th_corp_temp\" where \"corpID\" = '$corpID' and \"Approved\" = 'true' and \"hidden\" = 'false' ");
	while($res_lastedit = pg_fetch_array($query_lastedit))
	{
		$lastedit = $res_lastedit["maxedit"];
	}
	
	$query_chk_user = pg_query("select * from public.\"th_corp_temp\" where \"corpID\" = '$corpID' and \"corpEdit\" = '$lastedit' and \"Approved\" = 'true' and \"hidden\" = 'false' ");
	...
	if($RemarkAll != "")
	{
		$sql_no_appv_remarkall = "update public.\"th_corp_temp\" set \"RemarkAll\" = '$RemarkAll' 
								where \"corpID\" = '$corpID' and \"Approved\" is null and \"hidden\" = 'false' ";
...
elseif($appv == "1") // ถ้าอนุมัติ
{
	...
	$query_maxedit = pg_query("select max(\"corpEdit\") as \"maxedit\" from public.\"th_corp_temp\" where \"corpID\" = '$corpID' and \"Approved\" = 'true' and \"hidden\" = 'false' ");
	...
	// ข้อมูลนิติบุคคล
	$query_corp = pg_query("select * from public.\"th_corp_temp\" where \"corpID\" = '$corpID' and \"corpEdit\" = '$maxedit' and \"Approved\" = 'true' and \"hidden\" = 'false' ");
	while($result_corp = pg_fetch_array($query_corp))
	{
		...
		$sql_add_corp = "update public.\"th_corp\" set \"corpType\"=$corpType , \"corpName_THA\"=$corpName_THA , \"corpName_ENG\"=$corpName_ENG , \"trade_name\"=$trade_name , \"corp_regis\"='$corp_regis'
						,\"TaxNumber\"=$TaxNumber , \"phone\"=$phone , \"Fax\"=$Fax , \"mail\"=$mail , \"website\"=$website , \"date_of_corp\"=$date_of_corp , \"initial_capital\"=$initial_capital
						,\"authority\"=$authority , \"current_capital\"=$current_capital , \"asset_avg\"=$asset_avg , \"revenue_avg\"=$revenue_avg , \"debt_avg\"=$debt_avg , \"net_profit\"=$net_profit
						,\"date_of_last_data\"=$date_of_last_data , \"trends_profit\"=$trends_profit , \"BusinessType\"=$BusinessType , \"IndustypeID\"=$IndustypeID , \"explanation\"=$explanation
						,\"Proportion_in_country\"=$Proportion_in_country , \"Proportion_out_country\"=$Proportion_out_country , \"Proportion_Cash\"=$Proportion_Cash
						,\"Proportion_Credit\"=$Proportion_Credit , \"Amount_Employee\"=$Amount_Employee , \"CountryCode\" = $CountryCode
						where \"corpID\" = '$corpID' ";
	...
	// ลบที่อยู่เก่าออก
	$delete_adds = "delete from public.\"th_corp_adds\" where \"corpID\" = '$corpID' ";
	if($result_adds = pg_query($delete_adds))
	{}
	else
	{
		$status++;
	...
	$query_adds = pg_query("select * from public.\"th_corp_adds_temp\" where \"corpID\" = '$corpID' and \"addsEdit\" = '$maxedit' and \"Approved\" = 'true' and \"hidden\" = 'false' ");
	...
	// ลบบัญชีธนาคารเก่าออกก่อน
	$delete_bank = "delete from public.\"th_corp_acc\" where \"corpID\" = '$corpID' ";
	if($result_bank = pg_query($delete_bank))
	{}
	else
	{
		$status++;
	...
	// เพิ่มบัญชีใหม่
	$query_bank = pg_query("select * from public.\"th_corp_acc_temp\" where \"corpID\" = '$corpID' and \"accEdit\" = '$maxedit' and \"Approved\" = 'true' and \"hidden\" = 'false' ");
	...
	// ลบ ผู้รับมอบอำนาจ เก่าออกก่อน
	$delete_attorney = "delete from public.\"th_corp_attorney\" where \"corpID\" = '$corpID' ";
	if($result_attorney = pg_query($delete_attorney))
	{}
	else
	{
		$status++;
	}
	// เพิ่ม ผู้รับมอบอำนาจ ใหม่
	$query_attorney = pg_query("select * from public.\"th_corp_attorney_temp\" where \"corpID\" = '$corpID' and \"attorneyEdit\" = '$maxedit' and \"Approved\" = 'true' and \"hidden\" = 'false' ");
	...
	// ลบ กรรมการ เก่าออกก่อน
	$delete_board = "delete from public.\"th_corp_board\" where \"corpID\" = '$corpID' ";
	if($result_board = pg_query($delete_board))
	{}
	else
	{
		$status++;
	...
	// เพิ่ม กรรมการ ใหม่
	$query_board = pg_query("select * from public.\"th_corp_board_temp\" where \"corpID\" = '$corpID' and \"boardEdit\" = '$maxedit' and \"Approved\" = 'true' and \"hidden\" = 'false' ");
	while($result_board = pg_fetch_array($query_board))
	{
		$CusID = $result_board["CusID"]; // รหัสลูกค้า หรือ ชื่อลูกค้า
		$path_signature = $result_board["path_signature"]; // ที่เก็บไฟล์ตัวอย่างลายเซ็นต์
	...
	// ลบ ผู้ติดต่อ เก่าออกก่อน
	$delete_communicant = "delete from public.\"th_corp_communicant\" where \"corpID\" = '$corpID' ";
	if($result_communicant = pg_query($delete_communicant))
	{}
	else
	{
		$status++;
	}
	// เพิ่ม ผู้ติดต่อ ใหม่
	$query_communicant = pg_query("select * from public.\"th_corp_communicant_temp\" where \"corpID\" = '$corpID' and \"communicantEdit\" = '$maxedit' and \"Approved\" = 'true' and \"hidden\" = 'false' ");
	...
	// ลบ ผู้ถือหุ้น เก่าออกก่อน
	$delete_share = "delete from public.\"th_corp_share\" where \"corpID\" = '$corpID' ";
	if($result_share = pg_query($delete_share))
	{}
	else
	{
		$status++;
	}
	// เพิ่ม ผู้ถือหุ้น ใหม่
	$query_share = pg_query("select * from public.\"th_corp_share_temp\" where \"corpID\" = '$corpID' and \"shareEdit\" = '$maxedit' and \"Approved\" = 'true' and \"hidden\" = 'false' ");
...
{
	//ACTIONLOG
		$sqlaction = pg_query("INSERT INTO action_log(id_user, action_desc, action_time) VALUES ('$id_user', '(ALL) อนุมัติการแก้ไขข้อมูลลูกค้านิติบุคคล', '$logs_any_time')");
	//ACTIONLOG---
	pg_query("COMMIT");
...
else
{
	...
	echo "<form method=\"post\" name=\"form2\" action=\"frm_detail_appvEditCorp.php?corpID=$corpID\">";
