...
<body>
<?php
if($request!=1){ //กรณีไม่ได้ include ไฟล์นี้
...
if($prebillIDMaster==""){ //ถ้าส่งมาแสดงว่าต้องการให้แสดงทุกรายการที่เกี่ยวข้องกัน
	$condition="\"prebillID\"='$prebillID'";
}else{
	$condition="\"prebillIDMaster\"='$prebillIDMaster'";
}

if($stsprocess=='I'){ //ถ้าเป็นการเพิ่มข้อมูลจะยังไม่มี prebillID จะใช้เลข auto_id แทน
...
$qryadduser=pg_query("select a.\"dateAssign\",b.\"fullname\",a.\"addStamp\",\"taxInvoice\" from thcap_fa_prebill_temp a left join \"Vfuser\" b on a.\"addUser\" = b.\"id_user\" where $condition and \"edittime\"='$edittime' and a.\"stsprocess\" <> 'D' order by a.\"dateAssign\"");
...
<tr>
	<td>       
		...
		<div class="wrapper">
			...
			<?php
			$qryadd=pg_query("select \"dateAssign\",\"taxInvoice\" from thcap_fa_prebill_temp where $condition and \"edittime\"='$edittime' and \"stsprocess\" <> 'D' order by \"dateAssign\"");
			...
			<tr align="left" bgcolor="#EED5D2">
				...
				$qryfile=pg_query("SELECT \"file\"  from thcap_fa_prebill_file  where $val='$prebillIDMaster' and \"edittime\"='$edittime'");
