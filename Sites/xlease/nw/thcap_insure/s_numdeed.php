...
//ในตอนค้นหานี้ ต้องตรวจสอบด้วยว่าเลขที่โฉนดจะต้องไม่ซ้ำกับข้อมูลในระบบ
$qry_name=pg_query("select * from \"nw_securities\" a
left join \"nw_province\" b on a.\"proID\"=b.\"proID\" 
WHERE (cast(a.\"securID\" as character varying(20)) NOT IN(select c.\"refDeedContract\" from \"thcap_insure_checkchip\" c where \"statusApp\" not in('0','4') and \"statusInsure\"='0')) and (\"numDeed\" like '%$term%' or \"numLand\" like '%$term%' or condobuildingname like '%$term%' or condoregisnum like '%$term%' or district like '%$term%') ");
