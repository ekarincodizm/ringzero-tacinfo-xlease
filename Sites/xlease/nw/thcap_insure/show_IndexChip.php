...
<?php
	...
	while($reschip=pg_fetch_array($qrychip)){
		...
		//สถานะอนุมัติ
		if($statusApp=="0"){
			$txtapp="<font color=red>ไม่อนุมัติ</font>";
		}else if($statusApp=="1"){
			$txtapp="<font color=red>อนุมัติ</font>";
		}else{
			$txtapp="รออนุมัติ";
		}
		$i+=1;
		...
		echo "
			...
			if($statusApp==0){ //ให้รับทราบเพื่ีอให้หายไปจากหน้านี้
				echo "<td align=center><span style=\"cursor:pointer;\" onclick=\"if(confirm('ยืนยันการรับทราบ!!')){location.href='frm_IndexChip.php?auto_id=$auto_id&method=noapp'}\"><u>รับทราบ</u></span></td>";
