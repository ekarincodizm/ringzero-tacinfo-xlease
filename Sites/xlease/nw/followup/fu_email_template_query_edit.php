...
$temID=pg_escape_string($_POST['hdtemid']);
$header=pg_escape_string($_POST['header']);
$file = pg_escape_string($_POST['filehd']);
$edit = pg_escape_string($_POST['edit']);
$status = 0;
...
if($edit == 'edityes'){
	if($checksend == 'senduser'){
		...
		$id_user=pg_escape_string($_POST['id_user']);
				...
				$sendname=pg_escape_string($_POST['hdsendname']);
				$sendemail=pg_escape_string($_POST['hdsendemail']);
...
if(!empty($_FILES["fileup"]["name"])){
		...
		pg_query('BEGIN');			
		$strSQL = "update \"fu_template\" SET \"tem_name\"='$temp',\"tem_detail\"='$detail',\"tem_file\"='$file',\"tem_sendname\"='$sendname',\"tem_send_email\"='$sendemail',\"tem_encode\"='$encode',\"tem_header\"='$header' where \"temID\"='$temID'";
		$objQuery = pg_query($strSQL);	
			...
			if($objQuery){
					...
					$status++;
										...
										if($status ==0 ){					
												pg_query('COMMIT');	
												$strSQL = "select * from \"fu_template\" where \"temID\"='$temID'";
												$objQuery = pg_query($strSQL);	
												...
												$temID = $re1['temID'];
													...
													pg_query('ROLLBACK');
													echo "<script type='text/javascript'>alert('Can't save this template please try again')</script>";
													exit();	
										}		
	...
	pg_query('BEGIN');			
		$strSQL = "update \"fu_template\" SET \"tem_name\"='$temp',\"tem_detail\"='$detail',\"tem_sendname\"='$sendname',\"tem_send_email\"='$sendemail',\"tem_encode\"='$encode',\"tem_header\"='$header' where \"temID\"='$temID'";
		$objQuery = pg_query($strSQL);	
