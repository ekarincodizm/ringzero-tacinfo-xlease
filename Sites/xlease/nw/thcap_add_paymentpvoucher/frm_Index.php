...
function changepurpose(){
		...
		$.post('list_fromChannel.php',{voucherPurpose:voucherPurpose},function(data){	
		$('#fromChannel').html(data);
		});
	
		$.post('list_withMedium.php',{voucherPurpose:voucherPurpose},function(data){	
		$('#withMedium').html(data);		
		});
		//ช่อง from ช่องทางการรับเงิน:
		$.post('list_toChannel.php',{voucherPurpose:voucherPurpose},function(data){     
		$('#toChannel').html(data);
		});	
		selectChannel();
		selecttoChannel();
...
function selectChannel(){	
	...
	fromChannel=fromChannel[0];	
		$.post('list_fromChannelType.php',{fromChannel:fromChannel},function(data){		
...
function selecttoChannel(){	
	...
	$.post('list_fromChannelType.php',{fromChannel:toChannel},function(data){		
   ...
   $("#formula").change(function(){
    ...
    $("#btn_submit").click(function(){
		...
		if(document.getElementById("noaddFile").value < 1 ){
			alert('คุณยังไม่ได้ ทำรายการที่จะ บันทึกรายการChannel');
            return false;
		}
		var r=document.getElementsByName("made");
		var i=0;
...
<fieldset><legend><B>ทำรายการ- Payment Voucher</B></legend>
...
<?php 	include('frm_show_detail.php');?>
...
<div id="addpt" name="addpt">
	...
	<!--div> 
		<input type =checkbox name="showall1" id="showall1" onChange="showall();">แสดง/ซ่อน การบันทึก
	</div-->	
	<div id="purpost">
	<?php 	include('frm_save_channel.php');?>
...
<?php 
	...
	$sendfrom='1';
	include('../thcap_appv_pv/pv_wait.php');
	?>
</body>
</html>