...
<?php
	...
	//ใช้สูตร
		for($i=0;$i<count($_POST["text_accno1"]);$i++)
		{
			...
			$abd_bookType = pg_escape_string($_POST['text_drcr1'][$i]);
			if($abd_bookType=='1'){
				$abd_bookType_dr .='1,';
				$adds_money_dr.=$adds_money.',';
				$adds_serial_dr.=$adds_serial.',';
	...
	if(($chk_insert_channel=="on") and ($rowaddFile >0)){
		...
		values ('$date_add','$this_time','$user_id','$add_date',$f_id,'$f_name',$text_add,'1','9','$arrayaccbookserial','$arraybooktype','$arrayabd_amount','$voucherPurpose','$chk_to')
		...
		{
		//ACTIONLOG
			$sqlaction = pg_query("INSERT INTO action_log(id_user, action_desc, action_time) VALUES ('$user_id', '(THCAP)ทำรายการใบสำคัญจ่าย', '$add_date')");
		//ACTIONLOG---
			pg_query("COMMIT");
			//pg_query("ROLLBACK");			
		...
		pg_query("ROLLBACK");
		echo "ไม่สามารถเพิ่มข้อมูลได้ เนื่องจาก ยังไม่มีรายการ Channel";
	}
?>
</div>
</fieldset>
