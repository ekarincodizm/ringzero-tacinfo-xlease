<fieldset>				
	<div>
		...
		<tr style="font-weight:bold;" valign="top" bgcolor="#79BCFF" align="center">
			...
			<th>ค้างชำระไม่เกิน 1 เดือน</th>
			<th>เกินกำหนด 1 เดือน - 3 เดือน</th>
		...
		$sump0 = 0;
		$sump1b = 0; // ค้างชำระไม่เกิน 1 เดือน
		$sump1 = 0;
		$sump2 = 0;
		$sump3 = 0;
		$sump4 = 0;
		...
		for($con = 0;$con < sizeof($contypechk) ; $con++){
			...
			echo "<tr bgcolor=\"#FFD39B\"><td colspan=\"12\"><b>$contypechk[$con]</b></td></tr>";
			
			$i=0;
			$sumprinciple0 = 0;
			$sumprinciple1b = 0;  // ค้างชำระไม่เกิน 1 เดือน
			$sumprinciple1 = 0;
			$sumprinciple2 = 0;
			$sumprinciple3 = 0;
			$sumprinciple4 = 0;
			...
			while($result=pg_fetch_array($qrymg)){
				...
				$principle = 0.00;
				$principle0 = 0.00;
				$principle1b = 0.00;  // ค้างชำระไม่เกิน 1 เดือน
				$principle1 = 0.00;
				$principle2 = 0.00;
				$principle3 = 0.00;
				$principle4 = 0.00;
				...
				if($conclosedate == ''){ // ถ้าพบว่าสัญญายังไม่ปิดบัญชี คือ แสดงได้
					...
					if (		$credittype == "LOAN" OR 
						...
						// หาจำนวนเงินที่ถึงกำหนดชำระใน 1 งวดล่าสุดที่เพิ่งผ่านมา เนื่องจากจะต้องเป็นจำนวนเต็มเสมอถึงค้างได้เกิน 1 เดือน
						$qry_debtamt_1m=pg_query("	
													SELECT
														SUM(\"ptMinPay\")
													FROM
														account.\"thcap_payTerm\"
													WHERE
														\"contractID\" = '$contractID' AND
														\"ptDate\" >= ('$nowdate'::date - INTERVAL '1 month')::date AND
														\"ptDate\" <= '$nowdate'
						");
						list($debtamt_1m)=pg_fetch_array($qry_debtamt_1m);
						...
						$qry_debtamt_3m=pg_query("	
													...
													WHERE
														...
														\"ptDate\" < ('$nowdate'::date - INTERVAL '1 month')::date
						");
						...
						$qry_debtamt_1m=pg_query("	
													...
													WHERE
														\"contractID\" = '$contractID' AND
														\"duedate\" >= ('$nowdate'::date - INTERVAL '1 month')::date AND
														\"duedate\" <= '$nowdate'
						");
						list($debtamt_1m)=pg_fetch_array($qry_debtamt_1m);
						...
						$qry_debtamt_3m=pg_query("	
													...
													WHERE
														...
														\"duedate\" < ('$nowdate'::date - INTERVAL '1 month')::date
						");
						list($debtamt_3m)=pg_fetch_array($qry_debtamt_3m);
					...
					if($nubmonth=='structure' or ($issue==1 and $structure==1)){ // !!!!!!!!!!!!!!!!!!!! อยู่ระหว่างปรับโครงสร้างหนี้ !!!!!!!!!!!!!!!!!!!!
					...
					}else if($nubmonth <= 1){ //เกินกำหนด น้อยกว่า 3 เดือน
						$condition="1b";
						...
						$principle1b = $backamt_all;

					}else if($nubmonth > 1 and $nubmonth <= 3){ //เกินกำหนด น้อยกว่า 3 เดือน
						$condition="1";
						...
						===================================================================================================== */
						$principle0 = $principle - $backamt_all;
						$principle1b = $debtamt_1m;
						$principle1 = $backamt_all - $debtamt_1m;
						...
						$principle2 = $backamt_all - $debtamt_1m - $debtamt_3m;
						...
						$principle3 = $backamt_all - $debtamt_1m - $debtamt_3m - $debtamt_6m;
						...
						$principle4 = $backamt_all - $debtamt_1m - $debtamt_3m - $debtamt_6m - $debtamt_12m;
					}
					...
					if($principle1b!=""){
						$principle11b=number_format($principle1b,2);
					}else{
						$principle11b="";
					}
					if($principle1!=""){
					...
					*/
					if($condition=="0" || $condition=="1b" || $condition=="1" || $condition=="2" || $condition=="3" || $condition=="4"){echo $principle00;}else{ $principle0=0;}
					echo "</td><td align=right>";
					if($condition=="1b" || $condition=="1" || $condition=="2" || $condition=="3" || $condition=="4"){echo $principle11b;}else{ $principle1b=0;}
					echo "</td><td align=right>";
					...
					if($sumprinciple1b!=""){$sumprincipleshow1b=number_format($sumprinciple1b,2);}else{$sumprincipleshow1b="-";}
		...
		echo "<td>".number_format($sump1b,2)."</td>";
		...
		if($allsum>0){ //ป้องกันการ error จากการหารด้วย 0
			...
			$percent1b = number_format($sump1b/$allsum*100,2);
		...
		echo "<tr bgcolor=#FFCCCC><td colspan=9></td><td  align=center colspan=2><b>ลูกหนี้ทั้งสิ้น</b></td><td align=right><b>".number_format($allsum,2)."</b></td></tr>";
		
		if($numcontract==0){
			echo "<tr><td colspan=12 bgcolor=\"#E9F8FE\" align=center height=50><b>-ไม่พบรายการรับชำระ-</b></td></tr>";
		}
		?>
		</table>
	</div>
