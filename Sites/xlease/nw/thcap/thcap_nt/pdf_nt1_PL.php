...
// หาวันที่งวดแรก
$qry_startDueDate = pg_query("select \"ptDate\" from account.\"thcap_loan_payTerm_left\" where \"contractID\" = '$contractID' and \"ptNum\" = '$startnum' ");
$startDueDate = pg_fetch_result($qry_startDueDate,0);
$arrayfirst_unpaid = $startnum.",".$startDueDate;
$qry_startDueDate = pg_query("select get_date_thai_format('$startDueDate')");
list($dateThai_startDueDate)=pg_fetch_array($qry_startDueDate);

// หาวันที่งวดแรก
$qry_endDueDate = pg_query("select \"ptDate\" from account.\"thcap_loan_payTerm_left\" where \"contractID\" = '$contractID' and \"ptNum\" = '$endnum' ");
$endDueDate = pg_fetch_result($qry_endDueDate,0);
$arrayend_unpaid = $endnum.",".$endDueDate;
$qry_endDueDate = pg_query("select get_date_thai_format('$endDueDate')");
list($dateThai_endDueDate)=pg_fetch_array($qry_endDueDate);

// หาจำนวนเงินค้างชำระ
$qry_pay_amt = pg_query("select sum(\"ptMinPayLeft\") from account.\"thcap_loan_payTerm_left\" where \"contractID\" = '$contractID' and \"ptNum\" >= '$startnum' and \"ptNum\" <= '$endnum' ");
$pay_amt = pg_fetch_result($qry_pay_amt,0);
$arrayunpaid_detailall="";
$arrayunpaid_detailall="{"."ค่างวด".",".$pay_amt."}";

//----- งวดถัดไป
$nextnum = $endnum + 1;

// หาวันที่และจำนวนเงินงวดถัดไป
$qry_nextDueDate = pg_query("select \"ptDate\", \"ptMinPayLeft\" from account.\"thcap_loan_payTerm_left\" where \"contractID\" = '$contractID' and \"ptNum\" = '$nextnum' ");
list($nextDueDate, $next_ptMinPayLeft) = pg_fetch_array($qry_nextDueDate);

if($nextDueDate != "")
{
	$arraypay_next = $nextnum.",".$nextDueDate;
	$qry_nextDueDate = pg_query("select get_date_thai_format('$nextDueDate')");
	list($dateThai_nextDueDate)=pg_fetch_array($qry_nextDueDate);
}
...
$qr_get_lease_fine=pg_query("select \"thcap_get_loan_fine\"('$contractID','$daylease_fine')");
...
$sum_all = $sum_typePayAmt+$pay_amt+$next_ptMinPayLeft; // รวมเป็นเงินทั้งหมด
...
$ins="INSERT INTO thcap_pdf_nt( \"NT_ID\", \"contractID\", \"NT_Date\", \"Cus_main\", \"Guarantee\",  \"Cus_join\",
	...
	VALUES ('$ID_NT','$contractID','$nowdate','$name3','$guarantee','$Cus_join',
		'$arrayfirst_unpaid','$arrayend_unpaid','$pay_amt','$arrayunpaid_detailall',
		'$unpaid_detailall_amt','$conLoanIniRate','$arraypay_next','$next_ptMinPayLeft','$sum_all')";
if($resin=pg_query($ins)){
	$ntid = pg_fetch_result($resin,0); // NT
}else{
	$status++;
...
if($status==0){ 
	pg_query("COMMIT");
	echo "<script>window.location='pdf_reprint_nt1_PL.php?NT_ID=$ID_NT';</script>";
}
