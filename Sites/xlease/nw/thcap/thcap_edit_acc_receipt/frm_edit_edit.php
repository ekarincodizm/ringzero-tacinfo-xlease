...
$rowchk = $_GET['row'];
	...
	$qry_checkapp = pg_query("select * from finance.\"V_thcap_receive_transfer_tsfAppv\" where \"revTranStatus\" = '9' AND \"appvXID\" is null AND \"bankRevAccID\" = '$BIDshow' AND DATE(\"bankRevStamp\") = '$BDate' ");
	$row_checkapp = pg_num_rows($qry_checkapp);	
if($row_checkapp == $rowchk){	
?>
...
<?php 	$qry_bank = pg_query("SELECT * FROM finance.\"V_thcap_receive_transfer_tsfAppv\" where \"revTranStatus\" = '9' AND \"appvXID\" is null AND \"bankRevAccID\" = '$BIDshow' AND DATE(\"bankRevStamp\") = '$BDate' order by \"revTranID\" ");
...
<form name="f_list" id="f_list" action="process_edit.php" method="post">
...
<div style="float:left"><input type="button" value="ยืนยันข้อมูล" id="submitButton" onclick="chkValue();"></div>
<div style="float:right"></div>
<div style="clear:both"></div>
