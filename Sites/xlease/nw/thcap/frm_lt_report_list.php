...
<?php
$qry_name=pg_query("select auto_id,\"contractID\",\"sendDate\",\"cusName\",\"addressCon\",\"type_send\",\"regisnumber\",\"CusID\" from \"vthcap_letter\" 
where date(\"sendDate\") ='$date' $condition1 group by auto_id,\"contractID\",\"sendDate\",\"cusName\",\"addressCon\",\"type_send\",\"regisnumber\",\"CusID\" order by \"contractID\",\"sendDate\"");
...
while($res_name2=pg_fetch_array($qry_name)){
	...
	if($CusID!=""){//ตรวจสอบว่าที่อยู่ตรงกับที่ให้ไว้หรือไม่ (ถ้ามีค่าแสดงว่าเป็นที่อยู่ตามสัญญาซึ่งแก้ไขไม่ได้อยู่แล้วจึงไม่ได้เช็ค)
		...
		if($address!=$addressold){
			$txtalert="<font color=red>(จดหมายอาจถูกส่งไปที่อื่น ไม่ใช่ที่อยู่ที่ลูกค้าลูกค้าแจ้งไว้)</font>";
		}else{
			$txtalert="";
		}
	}
	...
	while($resdetail=pg_fetch_array($qrydetail)){
		if($resdetail['detailRef']==""){
			...
			if(pg_num_rows($qryreceipt)>0){ //แสดงว่าเป็นใบเสร็จ
				$ref="<span onclick=\"javascript:popU('Channel_detail.php?receiptID=$resdetail[detailRef]','','toolbar=no,menubar=no,resizable=no,scrollbars=yes,status=no,location=no,width=800,height=600')\" style=\"cursor: pointer;\"><font color=red>(<u>$resdetail[detailRef]</u>)</font></span>";
			}else{
				...
				if(pg_num_rows($qrytax)>0){ //แสดงว่าเป็นใบกำกับ
					$ref="<span onclick=\"javascript:popU('Channel_detail_v.php?receiptID=$resdetail[detailRef]','','toolbar=no,menubar=no,resizable=no,scrollbars=yes,status=no,location=no,width=800,height=600')\" style=\"cursor: pointer;\"><font color=red>(<u>$resdetail[detailRef]</u>)</font></span>";
				}else{
					...
					if(pg_num_rows($qrydebt)>0){ //แสดงว่าเป็นใบแจ้งหนี้
						$ref="<span onclick=\"javascript:popU('Channel_detail_i.php?debtInvID=$resdetail[detailRef]','','toolbar=no,menubar=no,resizable=no,scrollbars=yes,status=no,location=no,width=800,height=600')\" style=\"cursor: pointer;\"><font color=red>(<u>$resdetail[detailRef]</u>)</font></span>";
					}else{
...
?>
    <td align="center" valign="top"><span onclick="javascript:popU('../thcap_installments/frm_Index.php?show=1&idno=<?php echo $contractID;?>','','toolbar=no,menubar=no,resizable=no,scrollbars=yes,status=no,location=no,width=1100,height=800')" style="cursor:pointer;color:red;"><u><?php echo $contractID;?></u></span></td>
	...
	<?php
		...
		?>
			<input type="button" value="พิมพ์ใบเหลือง" onclick="window.open('print_yellow.php?cus_lid=<?php echo $auto_id?>&post=<?php echo $post?>')">
		<?php 
		}
		?>
	</td>
