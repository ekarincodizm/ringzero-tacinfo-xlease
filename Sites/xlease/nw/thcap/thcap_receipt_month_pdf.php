	...
	if($showtype!="2"){
	   ...
	   $pdf->SetFont('AngsanaNew','B',10);
		...
		if($receiptID!=$old_receiptID and $old_receiptID!="" and $showtype!="2"){ 
			...
			if(($typepay==$paytype_a and $paytype_a!="") || ($typepay==$paytype_b and $paytype_b!="")){
				$sumnet=$sumnet;
				$sumvat=$sumvat;
				$sum_amt=$sum_amt;
			}else{
		...
		if($showtype==1){ //กรณีเลือกประเภทเป็นชำระคืนเงินต้นหรือดอกเบี้ย
			...
			$nub+=1;
			if(($typepay==$paytype_a) and $paytype_a!=""){
				$cline += 5;
			}	
		...
		if($showtype=="0"){ //กำหนดให้แสดง record นี้กรณีไม่ได้เลือกชำระเงินต้นหรือดอกเบี้ย					
			...
			if($show==0 and ($typepay!=$paytype_b OR $paytype_b=="")){ //แสดงข้อมูลทั่วไป ยกเว้นที่เลือกประเภท "ดอกเบี้ย" และถ้าเงินต้นเป็น 0 จะไม่แสดง record ส่วนนี้ด้วย
				...
				//if($typePayID==$paytype and $contype!='LOAN' and $old_receiptID==""){ //(กรณี record ที่ 1)ถ้า $typepay ไม่เท่ากับเงินต้นและไม่ใช่ประเภท LOAN ให้เพิ่มแถวถัดไป
					 $cline += 5;
				//}
			}
		...
		if($receiveInterest >0 and $typepay==$paytype_b and $paytype_b!=""){
			$cline += 5;
		}
	} //end if	
}//end while
...
if($sum_all>0){
	...
	while($resstar=pg_fetch_array($qryredstar)){
		...
		if(($typepay==$paytype_a || $typepay==$paytype_b) and $paytype_a!="" and $paytype_b !=""){
			$sumnet=$sumnet;
			$sumvat=$sumvat;
			$sum_amt=$sum_amt;
		}else{
...
if($typepay2==""){ //กรณีเลือกแสดงทั้งหมด
	...
	if($numchkamt==0){ //แสดงว่าอาจเลือกแสดง type เป็น ชำระเงินต้น หรือ ดอกเบี้ย ทำให้ไม่มีในตาราง otherpay
		...
		if($type_b==$typepay2 and $type_b!=""){ //กรณีเป็นดอกเบี้ย
			...
			group by  \"byChannel\"");																
		}else if($type_a==$typepay2 and $type_a!=""){ //กรณีเป็นเงินต้น
