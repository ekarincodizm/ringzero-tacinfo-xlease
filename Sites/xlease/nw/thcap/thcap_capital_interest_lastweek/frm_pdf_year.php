...
while($resyear=pg_fetch_array($qry_year)){
	...
	$pdf->MultiCell(200,4,$buss_name,1,'C',0);
	$cline += 5;
	$nub+=1;
...
{											
	...
	$pdf->SetXY(5,38.5);
	$buss_name=iconv('UTF-8','windows-874',"ดอกเบี้ยรับรู้รายได้: ดอกเบี้ยคำนวณถึง ณ สิ้นวันของวันที่สิ้นเดือนของเดือนและปีที่เลือก หักด้วยที่ลูกค้าได้ชำระมาแล้ว");
	$pdf->MultiCell(200,4,$buss_name,0,'L',0);

	$pdf->SetXY(5,42.5);
	$buss_name=iconv('UTF-8','windows-874',"รวมคงเหลือ : เงินต้นคงเหลือรวมกับ ดอกเบี้ยรับรู้รายได้ในเดือนปีนั้นๆ");
	$pdf->MultiCell(200,4,$buss_name,0,'L',0);

	$pdf->SetXY(5,46.5);
	$buss_name=iconv('UTF-8','windows-874',"ดอกเบี้ยคงเหลือทั้งสัญญา : ดอกเบี้ยคงเหลือทั้งสัญญาหักด้วยดอกเบี้ยทั้งหมดที่ลูกค้าได้ชำระมาแล้ว (เฉพาะ สัญญาเช่า หรือเช่าซื้อ)");
	$pdf->MultiCell(200,4,$buss_name,0,'L',0);
	...
	$pdf->SetFont('AngsanaNew','B',10);
	$pdf->SetXY(75,52);
	$buss_name=iconv('UTF-8','windows-874',"ยอดสินเชื่อเริ่มแรก");
	$pdf->MultiCell(20,4,$buss_name,0,'C',0);
	...
	$pdf->SetFont('AngsanaNew','B',10);
	$pdf->SetXY(115,52);
	$buss_name=iconv('UTF-8','windows-874',"ดอกเบี้ยรับรู้รายได้");
	$pdf->MultiCell(20,4,$buss_name,0,'C',0);
	...
	$pdf->SetFont('AngsanaNew','B',10);
	$pdf->SetXY(155,52);
	$buss_name=iconv('UTF-8','windows-874',"จำนวนวันที่ค้าง");
	$pdf->MultiCell(20,4,$buss_name,0,'C',0);

	$pdf->SetXY(175,52);
	$buss_name=iconv('UTF-8','windows-874',"ดอกเบี้ยคงเหลือทั้งสัญญา");
...
$pdf->SetFont('AngsanaNew','B',10);	
$pdf->SetXY(5,$cline+2);
$buss_name=iconv('UTF-8','windows-874',"รวมทุกปี");
$pdf->MultiCell(70,4,$buss_name,0,'R',0);
