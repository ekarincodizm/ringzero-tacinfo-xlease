	...
	<td>
		...
		if($condition !=""){	
			...
			if($condition==1){ //แสดงข้อมูลที่ค้างทั้งหมด
				...
				where  \"RefAnyID\" is null group by \"IDNO\",\"IDCarTax\",\"nameone\",\"CusAmt\",\"TaxDueDate\",\"A_FIRNAME\",\"full_name\",\"carregis\",\"gasregis\" order by \"$sortby\" $sort");
			}else{ //แสดงข้อมูลที่ค้างประจำเดือน/ปี
				$query_main=pg_query("select \"IDNO\",\"IDCarTax\",\"nameone\",\"CusAmt\",\"TaxDueDate\",\"A_FIRNAME\",\"full_name\",\"carregis\",\"gasregis\" from \"VNwBillcar\" 
				where (EXTRACT(MONTH FROM \"TaxDueDate\")='$month' AND EXTRACT(YEAR FROM \"TaxDueDate\")='$year') and \"RefAnyID\" is null
