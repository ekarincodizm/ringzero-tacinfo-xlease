...
<tr>
	<td>
		<fieldset><legend><b>ตรวจสอบรายการย่อยผิดปกติ</b></legend>
		<form method="post" action="frm_abnormal_bill.php" name="form1">
			<table width="60%" border="0" align="center">
			<tr><td>
			<fieldset><legend><b>เลือกเงื่อนไขบริการที่ต้องการตรวจสอบ</b></legend>
			...
			<tr>
				<td align="center" height="30" colspan="2">
					...
					<?php
						$qry_inf=pg_query("select * from \"TypePay\" ORDER BY \"TypeID\" ASC");
						while($res_inf = pg_fetch_array($qry_inf)){
						?>
						<option value="<?php echo $res_inf["TypeID"];?>" <?php if($res_inf["TypeID"]==$typepay){?>selected<?php }?>><?php echo $res_inf["TName"];?></option>
						<?php
						}
						?>
					</select>
					<b>เงื่อนไข</b> 
					<select name="concom" id="concom">
						<option value=">" <?php if($concom == ">"){echo "selected";}?>>></option>
						<option value=">=" <?php if($concom == ">="){echo "selected";}?>>>=</option>
						<option value="<" <?php if($concom == "<"){echo "selected";}?>><</option>
						<option value="<=" <?php if($concom == "<="){echo "selected";}?>><=</option>
						<option value="=" <?php if($concom == "="){echo "selected";}?>>=</option>
					...
					<b>จำนวนเงิน</b> <input type="text" name="money" id="money" value="<?php echo $money;?>" onkeypress="return check_number(event);">
				</td>
			...
			<tr>
				<td align="right" height="30" width="50%">
					...
					<option value="1" <?php if($condition==1){ echo "selected";}?>>ข้อมูลทั้งหมด</option>
					<option value="2" <?php if($condition==2){ echo "selected";}?>>ข้อมูลประจำเดือน/ปี</option>
					</select>
				</td>
				<td align="left">
		...
		<tr style="font-weight:bold;" valign="middle" bgcolor="#79BCFF" height="25">
			<th align="center" <?php if($sortby2=="IDNO"){ echo "bgcolor=#ff6600";}?>"><a href="frm_abnormal_bill.php?type=1&condition=<?php echo $condition?>&month=<?php echo $month?>&year=<?php echo $year?>&sortby=IDNO&sort=<?php echo $sort;?>&typepay=<?php echo $typepay?>&concom=<?php echo $concom;?>&money=<?php echo $money;?>"><U>IDNO</U></a></th>
			<th align="center" <?php if($sortby2=="full_name"){ echo "bgcolor=#ff6600";}?>"><a href="frm_abnormal_bill.php?type=1&condition=<?php echo $condition?>&month=<?php echo $month?>&year=<?php echo $year?>&sortby=full_name&sort=<?php echo $sort;?>&typepay=<?php echo $typepay?>&concom=<?php echo $concom;?>&money=<?php echo $money;?>"><U>ชื่อผู้เช่าซื้อ</U></a></th>
			<th align="center" <?php if($sortby2=="carregis"){ echo "bgcolor=#ff6600";}?>"><a href="frm_abnormal_bill.php?type=1&condition=<?php echo $condition?>&month=<?php echo $month?>&year=<?php echo $year?>&sortby=carregis&sort=<?php echo $sort;?>&typepay=<?php echo $typepay?>&concom=<?php echo $concom;?>&money=<?php echo $money;?>"><U>ทะเบียนรถยนต์</U></a></th>
			<th align="center" <?php if($sortby2=="nameone"){ echo "bgcolor=#ff6600";}?>"><a href="frm_abnormal_bill.php?type=1&condition=<?php echo $condition?>&month=<?php echo $month?>&year=<?php echo $year?>&sortby=nameone&sort=<?php echo $sort;?>&typepay=<?php echo $typepay?>&concom=<?php echo $concom;?>&money=<?php echo $money;?>"><U>ประเภทค่าใช้จ่าย</U></a></th>
			<th align="center" <?php if($sortby2=="CusAmt"){ echo "bgcolor=#ff6600";}?>"><a href="frm_abnormal_bill.php?type=1&condition=<?php echo $condition?>&month=<?php echo $month?>&year=<?php echo $year?>&sortby=CusAmt&sort=<?php echo $sort;?>&typepay=<?php echo $typepay?>&concom=<?php echo $concom;?>&money=<?php echo $money;?>"><U>ยอดชำระ</U></a></th>
			<th align="center" <?php if($sortby2=="TaxDueDate"){ echo "bgcolor=#ff6600";}?>"><a href="frm_abnormal_bill.php?type=1&condition=<?php echo $condition?>&month=<?php echo $month?>&year=<?php echo $year?>&sortby=TaxDueDate&sort=<?php echo $sort;?>&typepay=<?php echo $typepay?>&concom=<?php echo $concom;?>&money=<?php echo $money;?>"><U>หนี้ประจำเดือน/ปี</U></a></th>		
		</tr>
		</thead>
		<?php
		if($sortby2 !=""){
		...
		if($condition !=""){	
			//แสดงข้อมูลหลักทั้งหมด
			//VNwBillcar จะเก็บข้อมูลที่ยังไม่มีเลขที่ใบเสร็จและมีข้อมูลย่อยเท่านั้น
			if($condition==1){ //แสดงข้อมูลทั้งหมด
				$query_main=pg_query("select \"IDNO\",\"IDCarTax\",\"nameone\",\"CusAmt\",\"TaxDueDate\",\"A_FIRNAME\",\"full_name\",\"carregis\",\"gasregis\",sum(\"TaxValue\") as \"TaxValue2\" from \"VNwBillcar\" 
			...
			while($result_main=pg_fetch_array($query_main)){
				...
				<?php
					//แสดงข้อมูลย่อย
				$query=pg_query("select DISTINCT \"nametwo\",sum(\"TaxValue\") as \"TaxValue\" from \"VNwBillcar\" where \"IDCarTax\"='$IDCarTax' group by \"nametwo\"");
				$i=0;
				$TaxValuesum=0;
			...
			<tr height="30" bgcolor="#FFFFFF">
				<td colspan="6" align="right">
					<form method="post" name="form2" action="pdf_abnormal_bill.php" target="_blank">
						...
						<input type="hidden" name="typepay" value="<?php echo $typepay;?>">
