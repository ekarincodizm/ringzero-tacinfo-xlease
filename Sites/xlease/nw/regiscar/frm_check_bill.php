...
<tr>
	<td>
		<fieldset><legend><b>ตรวจสอบรายการเบิกค่าใช้จ่ายเกินทุน</b></legend>
		<form method="post" action="frm_check_bill.php" name="form1">
		...
		<table width="100%" border="0" cellSpacing="1" cellPadding="3" align="center" bgcolor="#D0D0D0" class="sort-table">
		<thead>
		<tr style="font-weight:bold;" valign="middle" bgcolor="#79BCFF" height="25">
			<th align="center" <?php if($sortby2=="IDNO"){ echo "bgcolor=#ff6600";}?>"><a href="frm_check_bill.php?type=1&condition=<?php echo $condition?>&month=<?php echo $month?>&year=<?php echo $year?>&sortby=IDNO&sort=<?php echo $sort;?>"><U>IDNO</U></a></th>
			<th align="center" <?php if($sortby2=="full_name"){ echo "bgcolor=#ff6600";}?>"><a href="frm_check_bill.php?type=1&condition=<?php echo $condition?>&month=<?php echo $month?>&year=<?php echo $year?>&sortby=full_name&sort=<?php echo $sort;?>"><U>ชื่อผู้เช่าซื้อ</U></a></th>
			<th align="center" <?php if($sortby2=="carregis"){ echo "bgcolor=#ff6600";}?>"><a href="frm_check_bill.php?type=1&condition=<?php echo $condition?>&month=<?php echo $month?>&year=<?php echo $year?>&sortby=carregis&sort=<?php echo $sort;?>"><U>ทะเบียนรถยนต์</U></a></th>
			<th align="center" <?php if($sortby2=="nameone"){ echo "bgcolor=#ff6600";}?>"><a href="frm_check_bill.php?type=1&condition=<?php echo $condition?>&month=<?php echo $month?>&year=<?php echo $year?>&sortby=nameone&sort=<?php echo $sort;?>"><U>ประเภทค่าใช้จ่าย</U></a></th>
			<th align="center" <?php if($sortby2=="CusAmt"){ echo "bgcolor=#ff6600";}?>"><a href="frm_check_bill.php?type=1&condition=<?php echo $condition?>&month=<?php echo $month?>&year=<?php echo $year?>&sortby=CusAmt&sort=<?php echo $sort;?>"><U>ยอดชำระ</U></a></th>
			<th align="center" <?php if($sortby2=="TaxDueDate"){ echo "bgcolor=#ff6600";}?>"><a href="frm_check_bill.php?type=1&condition=<?php echo $condition?>&month=<?php echo $month?>&year=<?php echo $year?>&sortby=TaxDueDate&sort=<?php echo $sort;?>"><U>หนี้ประจำเดือน/ปี</U></a></th>	
			<th align="center" bgcolor="#FFBBFF">เลขที่ใบเสร็จ</th>
			<th align="center" bgcolor="#FFBBFF">ยอดที่ชำระ</th>
		</tr>
		</thead>
		<?php
		if($sortby2 !=""){
		...
		if($condition !=""){	
			...
			while($result_main=pg_fetch_array($query_main)){
				...
				//หาจำนวนเงินรวมของแต่ละ record
				$querysumTax=pg_query("select sum(\"TaxValue\") as sumtax from \"VNwBillcar\" where \"IDCarTax\" = '$IDCarTax'");
				if($ressum=pg_fetch_array($querysumTax)){
					$sumtax=$ressum["sumtax"];
				}
				
				//หาจำนวนเงินที่ลูกค้าจ่ายมา
				$queryPay=pg_query("select \"O_MONEY\",\"O_RECEIPT\" from \"FOtherpay\" where \"RefAnyID\"='$IDCarTax'");
				$numrowspay=pg_num_rows($queryPay);
				if($numrowspay == 0){
					$O_RECEIPT = "-ไม่พบ-";
					$O_MONEY = "0";
				}else{
					if($respay=pg_fetch_array($queryPay)){
						$O_RECEIPT = $respay["O_RECEIPT"];
						$O_MONEY = $respay["O_MONEY"];
					}
				}
				if($sumtax > $CusAmt){
				?>
					<tr bgcolor="#CEFFCE" style="font-weight:bold;">
						<td align="center"><span onclick="javascript:popU('../../post/frm_viewcuspayment.php?idno_names=<?php echo $IDNO; ?>&type=outstanding','<?php echo "$IDNO_outstanding"; ?>','toolbar=no,menubar=no,resizable=no,scrollbars=yes,status=no,location=no,width=1000,height=800')" style="cursor: pointer;" title="ดูตารางการชำระ"><u><?php echo $IDNO; ?></u></span></td>
						...
						<td align="center" bgcolor="#FFECFF"><?php echo $O_RECEIPT; ?></td>	
						<td align="right"bgcolor="#FFECFF"><?php echo number_format($O_MONEY,2); ?></td>		
					</tr>
					<?php
					//แสดงข้อมูลย่อย
					$query=pg_query("select * from \"VNwBillcar\" where \"IDCarTax\"='$IDCarTax'");
					$numrowsvn=pg_num_rows($query);
					$i=0;
					$TaxValuesum=0;
					while($result=pg_fetch_array($query)){
						...
						<td align="center" colspan="3" bgcolor="#FFECFF"></td>				
					</tr>
					<?php
					...
					?>
					<tr bgcolor="#FFFFEA"><td colspan="4" align="right"><b>รวมเงิน</b></td><td align="right" bgcolor="#33CC00"><b><?php echo number_format($TaxValuesum,2);?></b></td><td>&nbsp;</td><td colspan="2" bgcolor="#FFECFF">&nbsp;</td></tr>
					<?php
					$TaxValueallsum=$TaxValueallsum+$TaxValuesum; //รวมทั้งหมด
				}
			} //end while รายการหลัก
			?>
			<tr bgcolor="#79BCFF"><td colspan="4" align="right"><b><U>รวมเงินทุกรายการ</U></b></td><td align="right"><b><U><?php echo number_format($TaxValueallsum,2);?></U></b></td><td colspan="3">&nbsp;</td></tr>
			<?php
			if($i >0){
			?>
			<tr height="30" bgcolor="#FFFFFF">
				<td colspan="8" align="right">
					<form method="post" name="form2" action="pdf_check_bill.php" target="_blank">
