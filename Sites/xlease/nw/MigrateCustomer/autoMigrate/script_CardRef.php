...
$qry_main = pg_query("select distinct replace(replace(\"N_CARDREF\",' ',''),'-','') as \"mainCardID\",
					...
					where \"N_CARDREF\" is not null and \"N_CARDREF\" <> '' and \"N_CARDREF\" not like '%-' and \"statusapp\" = '1' and \"N_IDCARD\" is null
						and LENGTH(replace(replace(\"N_CARDREF\",' ',''),'-','')) = '13' and replace(replace(\"N_CARDREF\",' ',''),'-','') <> '3550600058492' and replace(replace(\"N_CARDREF\",' ',''),'-','') <> '0103551028922'
	...
	if(is_numeric($mainCardID)) // ถ้าบัตรที่หามาได้เป็นตัวเลขทั้งหมด
	{
		...
		if($nomrows_cusForMaxAppvDate == 1) // ถ้ามีแค่คนเดียวถึงจะทำ
		{
			...
			while($res_noCusID = pg_fetch_array($qry_noCusID))
			{
				...
				$test_sql4="insert into public.\"Fn_temp\" (\"CusID\",\"N_STATE\",\"N_SAN\",\"N_AGE\",\"N_CARD\",\"N_CARDREF\",\"N_OT_DATE\",\"N_BY\",\"N_OCC\",\"N_ContactAdd\",\"N_CARDREF\") values ('$noCusID' , '$N_STATE' , $N_SAN , '$N_AGE' , $N_CARD , $N_CARDREF , '$N_OT_DATE' , $N_BY , $N_OCC , $N_ContactAdd , $N_CARDREF )";
...
if($status == 0)
{
	...
	if($errorStr != ""){echo "<br><center>บัตรอื่นๆที่ไม่สามารถดำเนินการได้<br>$errorStr</center>";}
}
else
{
	pg_query("ROLLBACK");
