	...
	if($rows > 0 )
	{
		...
		if($cjkre2 == 5)
		{
			if(is_numeric($chkre))
			{
				for($m = 1 ; $m <= $a ; $m++)
				{
					$test_sql7="update $SCHEMA.\"$realtb\" set \"$column\"='$CusID3' where \"$column\"='$CusIDFn[$m]'";
...
for($m=1 ; $m <= $a ; $m++)
{
	$test_sql99="insert into public.\"change_cus\" (\"Cus_old\",\"Cus_new\") values ('$CusIDFn[$m]' , '$CusID3')";
...
if($status == 0)
{
	...
	echo "<form method=\"post\" name=\"form1\" action=\"V2_SearchCustomer.php\">";
