...
<?php 
			...
			if($id == ""){
				...
				if($row6 == 0){
					...
					$sql = pg_query("select * from \"temp_securities_detail\" where \"securdeID\" = '$idd'");
					$row = pg_num_rows($sql);
					...
					if($row == 0){
						...
						$sql1 = pg_query("select * from \"approve_securities_detail\" where \"securID\" = '$idd' and (\"status\" = '0' OR \"status\" = '2')");
...
<form name="frm" method="POST" action="estimate_edit_query.php" enctype="multipart/form-data">
<?php } ?>
...
<hr width="850">
	...
	<table width="850" cellSpacing="1" cellPadding="2"  align="center">
		...
		<tr>
			...
			<td colspan="4" bgcolor="#FFFFFF">
						...
						<?php
							...
							$qry_name9 = pg_query("select \"file\",\"securID\" from \"temp_securities_detail\" where \"securID\" = '$id'");					
		...
		<tr>
				<td></td>
				<td align="center" ><input type="submit" value=" ยืนยัน " style="width:100px; height:35px;" onclick="return checkList()"></td>
				<td align="center" colspan="2"><input type="button" value="ยกเลิก" style="width:100px; height:35px;" onclick="window.close()"></td>
		</tr>
