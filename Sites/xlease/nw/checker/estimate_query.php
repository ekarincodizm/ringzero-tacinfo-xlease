...
$Cus = $_POST['customer'];
list($CusID,$Cusname)=explode("#",$Cus);
$auditors = $_POST['auditors'];
list($auditorID,$auditorname)=explode("#",$auditors);
...
$result = pg_query($sql);
	...
	if($result){
		...
		if($status == 0){
			...
			$sql2=pg_query("insert into \"approve_securities_detail\"(\"securdeID\",\"status\",\"date\",\"securID\",\"id_user\") values($securdetailID,0,$datenow,'$securID','$id_user')");
					...
					if($status1 == 0){
							pg_query("COMMIT");
								//ACTIONLOG
									$sqlaction = pg_query("INSERT INTO action_log(id_user, action_desc, action_time) VALUES ('$id_user', '(THCAP) ทำการบันทึกตรวจสอบหลักทรัพย์', $datenow)");
								//ACTIONLOG---	
