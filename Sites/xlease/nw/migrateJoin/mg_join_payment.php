...
<?php			
$test_sql=pg_query("Truncate Table public.ta_join_payment "); //ลบข้อมูลในตารางเดิม
...
$test_sql=pg_query("select  ta_join_payment_id, car_license, contract_id,cpro_name, 
            ...
            payment_image, datetime, update_datetime, users, deleted from ta_tal_1r4_mg.\"ta_join_payment\" ");
$rowtest=pg_num_rows($test_sql);
$seq=1;
				...
				while($result5=pg_fetch_array($test_sql5))
{
	$id_user =$result5["id_user"];
					...
					list($car_license1,$car_license2)=explode("/",$car_license,2); 
...
if($amount_update_m!=0)$pay = "UD";
	...
	$test_sql4="INSERT INTO public.ta_join_payment(
