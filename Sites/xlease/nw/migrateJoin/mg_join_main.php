...
<?php  
$test_sql=pg_query("Truncate Table public.ta_join_main "); //ลบข้อมูลในตารางเดิม
$rowtest=pg_num_rows($test_sql);

//การตรวจสอบ เลขทะเบียนรถว่ามี รหัสรถยนต์เป็นอะไร
$test_sql=pg_query("select ta_join_pm_id, car_license, contract_id,cpro_name,id_body,deleted  from ta_tal_1r4_mg.\"ta_join_main\" ");
$rowtest=pg_num_rows($test_sql);
$seq=1;
...
$test_sql=pg_query("select id,ta_join_pm_id, car_license, contract_id,cpro_name,id_card,start_pay_date, staff_check,
 cancel, cancel_datetime, note, datetime, update_datetime, users, deleted from ta_tal_1r4_mg.\"ta_join_main\" order by id ");
$rowtest=pg_num_rows($test_sql);
$seq2=1;
while($result=pg_fetch_array($test_sql))
{
...
list($car_license1,$car_license2)=explode("/",$car_license,2); //แยก/ ออก
$car_license1  =trim($car_license1);
$id_body=trim($result["id_body"]);
...
//echo $id." ". strlen($id_card) . "<br>";

if($id_card=="" || $id_card=="-" || $id_card=="- " || $id_card=="--" || strlen($id_card)!=13){
$sql33 = "select \"CusID\" from public.\"Fa1\" where \"A_NAME\" = '$A_NAME' and  \"A_SIRNAME\" = '$A_SIRNAME' ";	
...
$sql33 ="select a.\"CusID\" from public.\"Fa1\" a inner join  public.\"Fn\" b on a.\"CusID\" = b.\"CusID\"  where a.\"A_NAME\" = '$A_NAME' and  a.\"A_SIRNAME\" = '$A_SIRNAME' and b.\"N_IDCARD\"='$id_card2' ";
...
$test_sql3=pg_query($sql33);
$rowtest3=pg_num_rows($test_sql3);
	if($rowtest3==0) //ถ้ายังไม่เจออีกให้ค้นหาเฉพาะ เลขบัตร
	$sql33="select a.\"CusID\" from public.\"Fa1\" a inner join  public.\"Fn\" b on a.\"CusID\" = b.\"CusID\"  where b.\"N_IDCARD\"='$id_card2' ";
...
/*if($member=="")$member_sql="NULL";
	else $member_sql = "'$member'";*/

if($note=="")$note_sql="NULL";
	else $note_sql = "'$note'";	
	
	if(trim($CusID)=="")$CusID_sql="NULL";
	...
	$test_sql4="INSERT INTO public.ta_join_main(
    ...
    VALUES  ('$id','$ta_join_pm_id','$car_license1',$CarID_sql,'$car_license_seq', '$contract_id' ,$CusID_sql,'$cpro_name',
            '$start_pay_date', '$staff_check', '$cancel', $cancel_datetime_sql, $note_sql, '$datetime', '$update_datetime', '$id_user', '$deleted')";
...
$test_sql=pg_query("delete from public.ta_join_main where deleted='1' "); //ลบข้อมูลในตารางเดิม
$rowtest=pg_num_rows($test_sql);

if($status == 0){
	$num_add++;
	$test_sql=pg_query("ALTER SEQUENCE ta_join_main_id_seq RESTART WITH $num_add "); //ลบข้อมูลในตารางเดิม
$rowtest=pg_num_rows($test_sql);
