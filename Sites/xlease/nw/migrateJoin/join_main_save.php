 ...
 function cus_save(i){
     ...
     document.getElementById('tr'+i).style.backgroundColor = "#97FFB9";
        $.post('CusSaveDB.php',{
            ...
            cus_id: document.getElementById('CusID'+i).value,   
...
<Th><b>นามสกุล</b></Th>
<Th><b>รหัสบัตร</b></Th>
<Th><b>รหัสลูกค้า CusID</b></Th>
<Th><b> บันทึก </b></Th>
</Tr>
<?php			

$test_sql=pg_query("select a.id,a.car_license, a.idno,a.cpro_name, a.deleted,b.id_card from public.\"ta_join_main\" a inner join ta_tal_1r4_mg.\"ta_join_main\" b on a.id= b.id where a.CusID is null ");
...
<TD><input type="text" id="CusID<?php echo $seq2; ?>"><input type="hidden" id="id_main<?php echo $seq2; ?>" value="<?php echo $id_main; ?>">
<input type="hidden" id="fname<?php echo $seq2; ?>" value="<?php echo $A_NAME; ?>">
<input type="hidden" id="lname<?php echo $seq2; ?>" value="<?php echo $A_SIRNAME; ?>">
</TD>
<TD><button onClick="cus_save(<?php echo $seq2; ?>)" style="width:100px;height:30px" > บันทึก </button></TD>
</TR>
		<?php		
		//$id_card2=null;	
		$seq2++;		
