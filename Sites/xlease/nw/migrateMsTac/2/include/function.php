...
function GetCusID(){
    $qry = pg_query("SELECT COUNT(\"cus_id\") AS countid FROM \"Customers2\"");
...
function GetCarID(){
    $qry = pg_query("SELECT COUNT(\"car_id\") AS countid FROM \"Cars2\"");
    $res = pg_fetch_array($qry);
    $res_count=$res['countid'];
    if($res_count == 0){
        $res_sn = 1;
...
function GetRadioID(){
    $qry = pg_query("SELECT COUNT(\"radio_id\") AS countid FROM \"Radios2\"");
    $res = pg_fetch_array($qry);
    $res_count=$res['countid'];
    if($res_count == 0){
        $res_sn = 1;
