...
<html>
<head>
    <title>(THCAP) ตรวจสอบผูกสัญญา</title>
...
<body>
<center><h2>(THCAP) ตรวจสอบผูกสัญญา</h2></center>
			...
			<tr bgcolor="#FFFFFF">
				<td colspan="10" align="left" height="25"><u><b>หมายเหตุ</b></u>
					...
					<div style="padding-top:5px;"><font color="red"><img src="images/button_ok.png" height="16" width="16">  รายการที่ฉันตรวจสอบแล้ว คือ รายการที่ตัวเองเป็นคนตรวจสอบ</font></div>
				</td>
			</tr>
			<table>
<br>
<table align="center" width="85%" border="0" cellspacing="1" cellpadding="1" bgcolor="#BBBBEE">
	<tr align="center" bgcolor="#79BCFF">
		...
		<th>รายละเอียด</th>
		<th>รายการที่ฉันตรวจสอบแล้ว</th>
		<th>จำนวนผู้ตรวจสอบ</th>
	</tr>
	<?php
	$namemenu="check";
	...
	while($result = pg_fetch_array($query))
	{
		...
		$Appquery = pg_query("select \"Approved\",\"appvID\" from public.\"thcap_contract_check_temp\" where \"ID\"=$contractAutoID and \"appvStamp\" = (select max(\"appvStamp\") from thcap_contract_check_temp where \"ID\"=$contractAutoID)");
			$Approved = pg_fetch_result($Appquery ,0);
			
			$countAppv = pg_query("select \"appvID\" from public.\"thcap_contract_check_temp\" where \"ID\"=$contractAutoID"); 
			$check=0;
			while($resAppv = pg_fetch_array($countAppv)){
				$appvID = $resAppv["appvID"];
				if ($appvID==$iduser){
					$check++;
				}
			}
			$countCheck = pg_num_rows($countAppv);
		...
		if($Approved!=""){
			...
			//ตรวจสอบว่าผู้ใช้งาน กับชื่อผู้ตรวจสอบเป็นคนเดียวกันหรือไม่
			if($check>0){
				$CheckImg = "<img src=\"images/button_ok.png\" height=\"20\" width=\"20\">";
			} else {
				$CheckImg = "";
			}
		}else {
		...
		if($contractType == 1)
		{
			echo "<td align=\"center\"><a onclick=\"javascript:popU('../loans_temp/frm_appv_financial_amount.php?contractAutoID=$contractAutoID&namemenu=$namemenu','','toolbar=no,menubar=no,resizable=no,scrollbars=yes,status=no,location=no,width=950,height=700')\" style=\"cursor:pointer;\"><font color=\"#0000FF\"><u>ตรวจสอบ</u></font></a></td>";
		}
		else
		{
			echo "<td align=\"center\"><a onclick=\"javascript:popU('../loans_temp/frm_appv_loan.php?contractAutoID=$contractAutoID&namemenu=$namemenu','','toolbar=no,menubar=no,resizable=no,scrollbars=yes,status=no,location=no,width=950,height=700')\" style=\"cursor:pointer;\"><font color=\"#0000FF\"><u>ตรวจสอบ</u></font></a></td>";
		}
		echo "<td align=\"center\">$CheckImg</td>";
		echo "<td align=\"center\">$countCheck</td>";
		echo "</tr>";
		$check=0;
	}
	if($numrows==0){
		...
		echo "<tr bgcolor=\"#79BCFF\" height=30><td colspan=13><b>ข้อมูลทั้งหมด $i รายการ</b></td><tr>";
	}
	?>
</table>
...
<?php
$showall = 'yes';
include("../loans_temp/frm_historyapp_limit.php");
?>
</body>
</html>