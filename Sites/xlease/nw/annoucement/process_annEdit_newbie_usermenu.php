...
if($chknewemp == '1'){ //แจ้งให้พนักงานใหม่ ทราบด้วย
			...
			if($newempdep==""){	
				...
				if($nummenunewbie == 0){ 
					$newbie="INSERT INTO nw_annouceuser_newbie(\"annId\", dep_id,\"statusAccept\")VALUES ('$annId', 'allemp','1')";
				}else{
					$newbie="UPDATE nw_annouceuser_newbie set dep_id='allemp',\"statusAccept\"='1' where \"annId\"='$annId' ";
				}		
			}else{
				
				if($nummenunewbie == 0){ 
					$newbie="INSERT INTO nw_annouceuser_newbie(\"annId\", dep_id,\"statusAccept\")VALUES ('$annId', '$newempdep','1')";
				}else{
					$newbie="UPDATE nw_annouceuser_newbie set dep_id='$newempdep',\"statusAccept\"='1' where \"annId\"='$annId' ";
				}
			}
			
			
}else{
	$newbie="update nw_annouceuser_newbie set \"statusAccept\" = '3' where \"annId\"='$annId'";	
}
...
if($status == 0){
	...
	echo "<meta http-equiv='refresh' content='2; URL=frm_annEdit_newbie_setupuser.php?ad_annId=$annId&a_gp=$a_gp'>";
}else{
	pg_query("ROLLBACK");
	echo $up_error."<br>";
	echo "<div style=\"padding: 50px 0px;text-align:center;\"><font size=4><b>ไม่สามารถบันทึกข้อมูลได้ กรุณาลองใหม่อีกครั้ง</b></font></div>";
	echo "<meta http-equiv='refresh' content='4; URL=frm_annEdit_newbie_setupuser.php?ad_annId=$annId&a_gp=$a_gp'>";
}	
?>