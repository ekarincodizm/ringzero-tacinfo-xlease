...
<body>
	<div style="margin-top:10px;" align="center"><h1>(THCAP) ใบสำคัญรับ</h1></div>
	...
	<fieldset><legend>เงือนไขการค้นหาหลัก</legend>
		<table align="center" cellspacing="10px">
			...
			<tr>
				<td><input type="radio" id="search_voucher" name="search_type"  value="0" /><!-- เลือกการค้นหา โดย ระบุ "ค้นหา Receive ID:" (เลขที่ใบสำคัญรับ) --></td>
				<td><b>ค้นหา Receive ID:</b></td>
				<td>
					<input type="text" name="s_receive" id="s_receive"><!-- นำเข้า Receive ID -->
				</td>	
			</tr>
			<tr>
				<td>
	...
	<!--   เริ่มส่วนปุ่มคำสั่งสำหรับ  "ค้นหา" ที่ให้ผู้ใช้ คลิก -->
	</div>	
	<table align="center" cellspacing="10px">
	  <tr>
	...
	<!-- สิ้นสุด ส่วนปุ่มคำสั่งสำหรับ  "ค้นหา" ที่ให้ผู้ใช้ คลิก-->
	<!--พื้นที่การแสดงการค้นหา-->
		<div id="list_receive" style="margin-top:10px;">
		</div>
	<!--/จบพื้นที่การแสดงการค้นหา-->	
	<div id="list_wait_cancel" style="width:80%;margin-top:30px;margin-left:auto;margin-right:auto;"><?php include("../thcap_approve_cancel_voucher/ReceiveVoucher_cancel_wait.php"); ?></div>
...
// ค้นหาที่ Receive ID เป็น autocomplete
$("#s_receive").autocomplete({
        source: "receive_autocomplete.php",
        minLength:1
});
// จบค้นหาที่ Receive ID เป็น autocomplete


$("#Search").click(function(){
	var chk = 0;
	var tv = $("#s_receive").val();
	...
	//ตรวจสอบการค้นหา ค้นหา Receive ID: 	
	...
	//จบการตรวจสอบการค้นหา ค้นหา Receive ID: 
	
	//ตรวจสอบการค้นหา ตามวันที่ :
	...
	//จบการตรวจสอบการค้นหา ตามวันที่ :
	...
	//จบการตรวจสอบการค้นหา ตามเดือน:
	
	//ตรวจสอบการค้นหา ตามช่วง
	...
	if(chk == 0){
		$("#list_receive").html('<img src="../../images/progress.gif" border="0" width="32" height="32" alt="กำลังโหลด...">');
		$("#list_receive").load("frm_list_receive.php",{
			txt_voucher:tv,
			s_date:date1,
			s_month:month,
			s_year:year,
...
$("input[name=search_type]").change(function(){
	...
	$("#dateto").val('');
	// $("#s_detail").val('')
	$("#year").val('<?php echo $v_year ?>');
	$("#sel_year").val('<?php echo $v_year ?>');
});
//จบการ เคลียค่า ข้อมูลต่าง ๆ
function popU(U,N,T) {
