...
$ForceID = pg_escape_string($_POST['ForceID']);
...
<body>
	<center>
		<div class="header"><h2>แก้ไข ข้อมูล ประกันภัย ภาคบังคับ (พรบ.)</h2></div>
		<div class="wrapper">
			<br>
			<?php
			...
			else
			{
				...
				$in_sql = "
							...
							values(
								...
								'$add_date',
								'9',
								(select count(*) from \"insure\".\"thcap_InsureForce_request\" where \"ForceID\" = '$ForceID' and \"editTime\" > '0') + 1,
								'$ForceID'
							)";
			...
			if($status==0){
				pg_query("COMMIT");
				//ACTIONLOG
						$sqlaction = pg_query("INSERT INTO action_log(id_user, action_desc, action_time) VALUES ('$add_user','(THCAP) แก้ไข ข้อมูล ประกันภัย ภาคบังคับ (พรบ.)', '$add_date')");
				//ACTIONLOG---
