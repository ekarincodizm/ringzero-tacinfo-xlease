...
if($rdo_search=='1')
{
	...
	//--- gen function เพื่อสร้างข้อมูล
		...
		$sqlaction = pg_query("select warehouse.thcap_update_wh_r_cash('$by_month_date', '$user_id')");
	...
	//select ข้อมูล
	$qry = pg_query("select * from warehouse.thcap_wh_r_cash_details where EXTRACT(MONTH FROM \"thcap_wh_r_cash_date\") = '$by_month_month' 
	AND EXTRACT(YEAR FROM \"thcap_wh_r_cash_date\") = '$by_month_year' order by \"thcap_wh_r_cash_date\" ");
}
else if($rdo_search=='2')
{
	...
	//--- gen function เพื่อสร้างข้อมูล
		...
		$sqlaction = pg_query("select warehouse.thcap_update_wh_r_cash('$by_year_date', '$user_id')");
		if($sqlaction){}else{$status++;}
	...
	//select ข้อมูล
	$qry = pg_query("select * from warehouse.thcap_wh_r_cash_details where EXTRACT(YEAR FROM \"thcap_wh_r_cash_date\") = '$by_year_year' order by \"thcap_wh_r_cash_date\"");	
}
...
<fieldset><legend><B>รายละเอียด</B></legend>
	...
	<table width="90%" cellpadding="5" cellspacing="1" border="0" bgcolor="#D0D0D0">
		<tr height="25" bgcolor="#79BCFF">
			...
			<th width="12%">เงินสดคงเหลือ ณ สิ้นวัน</th>
		...
		<?php
			...
			{	$i++;
				$thcap_wh_r_cash_date =$res_qry["thcap_wh_r_cash_date"] ;// วันที่รายงานเงินสด
				$thcap_wh_r_cash_yesterdayamt=$res_qry["thcap_wh_r_cash_yesterdayamt"]  ;//  เงินสดคงเหลือยกมา
				$thcap_wh_r_cash_sumrecamt=$res_qry["thcap_wh_r_cash_sumrecamt"] ;// จำนวนเงินสดรวมรับทั้งวัน
				$thcap_wh_r_cash_sumpayamt=$res_qry["thcap_wh_r_cash_sumpayamt"]  ;//  จำนวนเงินสดรวมจ่ายทั้งวัน
				$thcap_wh_r_cash_todayamt=$res_qry["thcap_wh_r_cash_todayamt"]  ;//  เงินสดคงเหลือ ณ สิ้นวัน
				$thcap_wh_r_cash_changeamt=$res_qry["thcap_wh_r_cash_changeamt"]  ;//  เงินสดที่เปลี่ยนแปลง (วันนี้ลบด้วยเมื่อวาน)
				...
				echo "<td align=center>$thcap_wh_r_cash_date</td>"; 
				echo "<td align=right>".number_format($thcap_wh_r_cash_yesterdayamt,2)."</td>";
				echo "<td align=right>".number_format($thcap_wh_r_cash_sumrecamt,2)."</td>";
				echo "<td align=right>".number_format($thcap_wh_r_cash_sumpayamt,2)."</td>";
				echo "<td align=right>".number_format($thcap_wh_r_cash_todayamt,2)."</td>";
				echo "<td align=right>".number_format($thcap_wh_r_cash_changeamt,2)."</td>";
				echo "<td align=\"center\"><img src=\"images/detail.gif\" width=\"19\" height=\"19\" onclick=\"javascript:popU('frm_list_show.php?datepicker=$thcap_wh_r_cash_date','','toolbar=no,menubar=no,resizable=no,scrollbars=yes,status=no,location=no,width=1000,height=700');\" style=\"cursor:pointer;\"></td>";
			}
			if($numrow==0){
