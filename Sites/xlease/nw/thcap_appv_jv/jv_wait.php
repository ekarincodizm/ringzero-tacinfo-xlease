...
$qry_app = pg_query("select * from thcap_temp_voucher_pre_details where \"voucherType\" ='3' and \"appvStatus\" = '9' order by $orderby $sort ");
...
<html>
<head>
	<title>ใบสำคัญรายวันทั่วไปที่รออนุมัติ</title>
...
<body>
	<table width="100%" border="0" cellSpacing="1" cellPadding="3" align="center" bgcolor="#F0F0F0">
		<tr bgcolor="#FFFFFF">
			<td colspan="11" align="left" style="font-weight:bold;">ใบสำคัญรายวันทั่วไปที่รออนุมัติ</td>
		</tr>
		...
		?>
			...
			<?php if($sendfrom=='1') {?>
			<td><span onclick="javascript:popU('../thcap_appv_jv/jv_detail.php?page=Y&autoID=<?php echo $prevoucherdetailsid?>','','toolbar=no,menubar=no,resizable=no,scrollbars=yes,status=no,location=no,width=1000,height=700')" style="cursor:pointer;"><u>ตรวจสอบ</u></span></td>
			<?php }else{?>
			<td><span onclick="javascript:popU('jv_detail.php?page=Y&autoID=<?php echo $prevoucherdetailsid?>','','toolbar=no,menubar=no,resizable=no,scrollbars=yes,status=no,location=no,width=1000,height=700')" style="cursor:pointer;"><u>ตรวจสอบ</u></span></td>
			<?php } ?>
		</tr>
		<?php
		} //end while
