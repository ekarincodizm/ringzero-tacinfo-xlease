...
$status = 0;
	...
	if($lostData>0){
		...
		if($status>0){
			...
			//ACTIONLOG
				if($sqlaction = pg_query("INSERT INTO action_log(id_user, action_desc, action_time) VALUES ('$user_id', '(THCAP) อนุมัติใบสำคัญรายวันทั่วไป', LOCALTIMESTAMP(0))")); else $status++;
			//ACTIONLOG---
			
			if($appv_status == "1"){?>			
			<form name ="frm1" action="../thcap_journal_voucher/pdf_journal_voucher.php" method="post"  target="_blank">
