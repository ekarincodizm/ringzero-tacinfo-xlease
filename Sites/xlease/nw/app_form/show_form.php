...
<div align="center">
    ...
    <div class="history">
    	<div>
    		<span class="btn reply" id="show_history">แสดงประวัติ</span>
        </div>
        <ul id="ul_history">
        <?php
			$sql = "select \"formid\",\"doer_time\" from $schema.\"app_frm_log\" where \"formid\"='$id'";
			$db_query = pg_query($sql);
			$row=pg_num_rows($db_query);
			if($row==0)
			{
				echo "<li class=\"list_history\">ยังไม่มีประวัติ</li>";
			}
			else
			{
				while($res=pg_fetch_array($db_query))
				{
					echo "<li class=\"list_history\"><a href=\"show_history.php?id=".$res['formid']."&dotime=".$res['doer_time']."\" target=\"_blank\">".$res['doer_time']."</a></li>";
				}
			}
		?>
        </ul>
	...
	<div class="container">
                    ...
                    <div class="inline two_col">
                        ...
                        <div class="row">
                            ...
                            <div class="input">
                                ...
                                <label class="inline"><input type="radio" name="owner" id="owner1" value="ผู้ขาย" <?php if($rs['owner']=="ผู้ขาย") echo "checked=\"checked\""; ?> /> ผู้ขาย</label>
        ...
        <div class="row">
        	<div class="box" id="block6">
                <div class="row center gray bold medium">คำถาม - คำตอบ</div>
                <div class="input">
                	<?php
					$q_question = "select * from $schema.\"questions\" where \"formID\"='$id'";
					$qr_question = pg_query($q_question);
					while($rs_question = pg_fetch_array($qr_question))
					{
						$questionID = $rs_question['questionID'];
						$content = $rs_question['content'];
						$poster = $rs_question['poster'];
						
						$q_user = "select \"app_fullname\" from $schema.\"app_member\" where \"memberID\"='$poster'";
						$qr_user = pg_query($q_user);
						$rs_user = pg_fetch_array($qr_user);
						$doer_name = $rs_user['app_fullname'];
						
						echo "
							<div class=\"row\">
								<div class=\"question_box\">
									<div class=\"quester inline-block\">$doer_name<br/>(THCAP)</div>
									<div class=\"question inline-block\">$content</div>
								</div>
							</div>
						";
						
						$q_answer = "select * from $schema.\"answers\" where \"questionID\"='$questionID'";
						$qr_answer = pg_query($q_answer);
						while($rs_answer = pg_fetch_array($qr_answer))
						{
							$answer = $rs_answer['answer'];
							$answer_by = $rs_answer['answer_by'];
							
							$q_user = "select \"app_fullname\" from $schema.\"app_member\" where \"memberID\"='$answer_by'";
							$qr_user = pg_query($q_user);
							$rs_user = pg_fetch_array($qr_user);
							$doer_name = $rs_user['app_fullname'];
							...
							echo "
								<div class=\"row\">
									<div class=\"question_box\">
										<div class=\"ans inline-block\">$answer</div>
										<div class=\"answer inline-block\">$doer_name<br />(ผู้ขาย)</div>
									</div>
								</div>
							";
						}
        ...
        <div class="row">
        	<div class="box" id="block6">
            	<form name="feedback_frm" id="feedback_frm" action="feedback_process.php?id=<?php echo $id; ?>" method="post">
                	<input type="hidden" name="formID" id="formID" value="<?php echo $id; ?>" />
                    <div class="row center gray bold medium">รายละเอียดการแจ้งปรับปรุง</div>
                    <div class="input">
                        <textarea name="txtarea_edit_details" id="txtarea_edit_details"></textarea>
                    </div>
                </form>
            </div>
        </div>
        <div class="bottom_form">
        	<div style="text-align:left; width:49%;" class="inline-block">
            	<span class="btn reply" onclick="window.location.href = 'admin.php';">กลับ</span>
            </div>
            <div style="text-align:right; width:49%;" class="inline-block">
            	<span class="btn save" onclick="mark('$id','1','อนุมัติ')">อนุมัติ</span>
                <span class="btn save" onclick="mark('$id','2','ไม่อนุมัติ')">ไม่อนุมัติ</span>
            	<span class="btn save" onclick="feedback();">แจ้งปรับปรุง</span>
            </div>
        </div>
</div>
</div>
...
$(document).ready(function(){
	...
	$('#show_history').click(function(){
		var opt = $('#show_history').html();
		if(opt=='แสดงประวัติ')
		{
			$('#ul_history').show();
			$('#show_history').html('ซ่อนประวัติ');
		}
		else
		{
			$('#ul_history').hide();
			$('#show_history').html('แสดงประวัติ');
		}
	});
	CKEDITOR.replace('txtarea_edit_details',{
		uiColor: '#fefefe'
	});
});
...
function feedback(){
	var content = CKEDITOR.instances.txtarea_edit_details.getData();
	if(content=='')
	{
		alert('โปรดระบุรายละเอียดที่ต้องการให้ปรับปรุง');
	}
	else
	{
		$('#feedback_frm').submit();
