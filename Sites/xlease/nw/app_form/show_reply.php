...
<div align="center">
	...
	<div class="container">
        ...
        <div class="reply_box">
        	<div class="row bold big bottom_dot" id="title"><?php echo $rs['title']; ?></div>
            <div class="row"><?php echo $rs['content']; ?></div>
            <?php
        ...
        <div class="row">
        	<form name="answer" method="post" action="proc/answer_process.php">
            	<input type="hidden" name="reply_id" id="reply_id" value="<?php echo $id; ?>" />
        		<textarea name="answer" id="answer" class="full"></textarea>
                <div class="row bold">เลือกสถานะ : </div>
                <div class="row">
                	<select name="pick_status" id="pick_status" class="middle_short">
                    <?php
					$q2 = "select * from $schema.\"status\" where \"status_type\"='2' order by \"statusID\"";
					$qr2 = pg_query($q2);
					if($qr2)
					{
						while($rs2 = pg_fetch_array($qr2))
						{
							echo "
								<option value=\"".$rs2['statusID']."\">".$rs2['status_name']."</option>
							";
						}
					}
					?>
