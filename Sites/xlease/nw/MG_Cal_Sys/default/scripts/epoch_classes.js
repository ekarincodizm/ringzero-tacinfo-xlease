...
function Epoch(name,mode,targetelement,multiselect)
{
	...
	if(this.mode == 'popup' && targetelement && targetelement.type == 'text') //if the target element has been set to be an input text box
	{
		...
		this.topLIMIT = this.tgt.LIMITHeight; // the vertical distance (in pixels) to display the calendar from the Top of its input element
		this.leftLIMIT = 0; 					// the horizontal distance (in pixels) to display the calendar from the Left of its input element
		this.calendar.style.top = this.getTop(targetelement) + this.topLIMIT + 'px';
		this.calendar.style.left = this.getLeft(targetelement) + this.leftLIMIT + 'px';
...
Epoch.prototype.calConfig = function () //PRIVATE: initialize calendar variables
{
	...
	this.rangeYearLower = 1944;
	this.rangeYearUpper = 2037;
	this.minDate = new Date(1944,0,1);
...
{
	this.calendar.style.top = this.getTop(target) + this.topLIMIT + 'px'
	this.calendar.style.left = this.getLeft(target) + this.leftLIMIT + 'px'
}
//-----------------------------------------------------------------------------
...
{
	return parseInt(Math.floor((this.getTime() - this.getTimezoneLIMIT() * 60000)/86400000)); //must take into account the local timezone
};
//-----------------------------------------------------------------------------
