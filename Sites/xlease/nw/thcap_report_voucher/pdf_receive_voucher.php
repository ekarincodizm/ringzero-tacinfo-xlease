...
$select_print_RV = $_POST["select_print_RV"];
...
//ACTIONLOG
if($sqlaction = pg_query("INSERT INTO action_log(id_user, action_desc, action_time) VALUES ('$doer', '(THCAP) ใบสำคัญรับ', LOCALTIMESTAMP(0))")); else $status++;
//ACTIONLOG---
}
...
//------------------------------------------------- รายละเอียด ---------------------------------------------------------------------
	...
	for($a=0;$a<=0;$a++){
		...
		for($j=0;$j<=0;$j++){
			...
			$pdf->SetFont('AngsanaNew','B',14);
			$pdf->SetXY(157,$n);
			$title=iconv('UTF-8','windows-874',"รายวันรับ
			RECEIVE VOUCHER");
