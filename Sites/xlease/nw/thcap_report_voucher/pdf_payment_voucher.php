...
$select_print_PV = $_POST["select_print_PV"];
...
//ACTIONLOG
if($sqlaction = pg_query("INSERT INTO action_log(id_user, action_desc, action_time) VALUES ('$doer', '(THCAP) พิมพ์ใบสำคัญจ่าย', LOCALTIMESTAMP(0))")); else $status++;
//ACTIONLOG---
}
...
for($i=0;$i<count($select_print_PV);$i++){
	
	$pdf->AddPage();
	
	$voucherID[$i]=$select_print_PV[$i];
	
	$qry = "select * from v_thcap_temp_voucher_details_payment where \"voucherID\"='$voucherID[$i]'";
...
//------------------------------------------------- รายละเอียด ---------------------------------------------------------------------
	...
	for($a=0;$a<=0;$a++){
		...
		for($j=0;$j<=0;$j++){
			...
			$pdf->SetFont('AngsanaNew','B',14);
			$pdf->SetXY(157,$n);
			$title=iconv('UTF-8','windows-874',"รายวันจ่าย
			PAYMENT VOUCHER");
			...
			$n += 6;
			$pdf->SetXY(5,$n);
			$title=iconv('UTF-8','windows-874',"ผู้จ่าย
			Financial office");
			$pdf->MultiCell(50,4,$title,0,'C',0);
