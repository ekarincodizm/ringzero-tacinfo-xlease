...
<head>
	...
	<script>
		...
		{
			if(voucherType == 'PV') // ใบสำคัญจ่าย
			{
				frm_search_PV.submit();
			}
			else if(voucherType == 'RV') // ใบสำคัญรับ
			{
...
<body>
	...
	if($s_data == "yes") // ถ้ามีการคลิกค้นหา
	{
		...
		if($s_valuee=="0"){ // ค้นหา "Voucher ID"
			...
			$Cnd1 = " where EXTRACT(YEAR FROM a.\"doerStamp\")='$s_sel_year' $condition_c ";
		...
		<div align="center">
			<fieldset style="width:80%;">
				<legend>
					<input type="button" value="รายการใบสำคัญจ่าย" style="height:4em; cursor:pointer;" onClick="load_voucher('PV');">
					<input type="button" value="รายการใบสำคัญรับ" style="height:4em; cursor:pointer;" onClick="load_voucher('RV');" >
					<input type="button" value="รายการใบสำคัญรายวันทั่วไป" style="height:4em;" disabled>
				</legend>
				<form name="frm_JV" action="pdf_payment_voucher.php" method="post" target="_blank">
						...
						<table cellpadding="5" cellspacing="0" border="0" width="100%" bgcolor="#F0F0F0" align="center">
							<tr bgcolor="white">
								...
								<td colspan="7" align="right">
									<input type="button" name="cancel" id="cancel" value="ขอยกเลิก ใบสำคัญรายวันทั่วไป" onclick="validate_JV(this.form,'C');"/>
									<input type="button" name="PrintPDF" id="PrintPDF" value="PrintPDF ใบสำคัญรายวันทั่วไป" onclick="validate_JV(this.form,'PDF');"/>
								</td>
							</tr>
							<tr style="font-weight:bold;" valign="middle" bgcolor="#BEBEBE" align="center">
								...
								<td><span id="selectAll_J" style="cursor:pointer;"><u><font color="blue">เลือกรายการ</font></u></span></td>
								<td>หมายเหตุ</td>
							</tr>
							<?php 
								...
								if($num_row>0){
									...
									if($i%2==0){	
										...
										if($voucherStatus == "0"){
											...
											echo "<td align=\"center\"><input type=\"checkbox\" name=\"select_print_JV[]\" id=\"select_print_JV$i\" value=\"$voucherID\"></td>";
