...
{
	$DueNo = $result["DueNo"]; // งวดที่  
	$ptDate = $result["duedate"]; // วันที่ครบกำหนดชำระ 
	$receiveDate = $result["receiveDate"]; // วันที่จ่าย 
	$delay = $result["delay"]; // จำนวนวันล่าช้า 
	$receiptID = trim($result["receiptID"]); //เลขที่ใบเสร็จ
	$taxinvoiceID = trim($result["taxinvoiceID"]); //เลขที่ใบกำกับภาษี
	$debtall_cut = number_format($result["debtall_cut"],2); //ค่างวดรวม VAT
	$typePayLeft = number_format($result["typePayLeft"],2); // ยอดที่ต้องชำระรวม VAT	
	$totaldebtall_before = number_format($result["totaldebtall_before"],2); //ยอดคงเหลือก่อนชำระรวม VAT
	$totaldebt_before = number_format($result["totaldebt_before"],2); //ยอดคงเหลือก่อนชำระไม่รวม VAT
	$totaldebtall_left = number_format($result["totaldebtall_left"],2); //ยอดคงเหลือหลังชำระไม่รวม VAT
	$totaldebt_left = number_format($result["totaldebt_left"],2); //ยอดคงเหลือหลังชำระไม่รวม VAT
...
// จบการหาค่าจาก function ใน postgres
	...
	if($signDate != "")
	{
		...
		$LastLeftPrinciple = pg_fetch_result($qry_Installment,0);
		if($LastLeftPrinciple == ""){$LastLeftPrinciple = 0.00;}
		...
		$sum_close_account = $LastLeftPrinciple + $plusone + $plustwo + $casa; // เงินรวม
		...
		$pdf->SetXY(5,$cline);
		$buss_name=iconv('UTF-8','windows-874',"ยอดเงินค่าซาก");
		$pdf->MultiCell(60,4,$buss_name,0,'L',0);
		
		$pdf->SetXY(65,$cline);
		$buss_name=iconv('UTF-8','windows-874',number_format($casa,2));
		$pdf->MultiCell(20,4,$buss_name,0,'R',0);
