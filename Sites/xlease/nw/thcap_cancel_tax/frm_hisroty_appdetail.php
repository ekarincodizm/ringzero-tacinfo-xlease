...
<center><h1>ประวัติการอนุมัติยกเลิกใบกำกับภาษี</h1></center>
		...
		<table width="100%" border="0" cellSpacing="1" cellPadding="3" align="center" bgcolor="#FFFFFF">
			...
			while($res_app=pg_fetch_array($qry_app)){
				...
				if($nubchk>0){ //แสดงว่ายังไม่ถูกลบ
					$qry_receipt2=pg_query("	
												SELECT a.\"cancelTaxID\",a.\"contractID\",a.\"taxinvoiceID\",\"fullname\",\"requestDate\",e.\"taxpointDate\",f.\"receiveAmount\",e.\"debtID\",e.\"typePayID\"
												FROM \"thcap_temp_taxinvoice_cancel\" a
			...
			?>
				...
				<!--<td align="center" style="color:#0000FF;"><span onclick="javascript:popU('Channel_detail.php?taxinvoiceID=<?php echo $taxinvoiceID; ?>','','toolbar=no,menubar=no,resizable=no,scrollbars=yes,status=no,location=no,width=800,height=600')" style="cursor: pointer;"><u><?php echo $taxinvoiceID; ?></u></span></td>-->		
