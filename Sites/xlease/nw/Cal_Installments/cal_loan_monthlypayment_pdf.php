...
//- หาจำนวนเดือนที่ต้องผ่อน *********************************************************************************************************************************************************************************************************	
$min_pay = $_GET["moneypay"]; //ยอดผ่อนขั้นต่ำ
$first_pay_date = $_GET['datestart']; //วันที่เริ่มจ่าย		
$start_date = $_GET["datestartcon"]; //วันที่ทำสัญญา
...
$min_pay_neg = ($min_pay)*(-1);//จำนวนเงินเต็มสำหรับดักให้ยอดผ่อนงวดสุดท้ายไม่เกิน การจ่ายของค่างวด เช่น ค่างวด 5000 จะต้องมีส่วนเกินได้ไม่เกิน 5000 ในการจ่ายครั้งสุดท้าย
while($last < $min_pay_neg ){
	$length = $length - 1;
	$last = func_mort_check_valid_accounting_test($credit,$int_normal,$start_date,$payday,$length,$min_pay,$first_pay_date);		
}
$time = $length;
$period = $min_pay;
$period_show = number_format($min_pay,2);
			
//- จบการหาจำนวนเดือนที่ต้องผ่อน *********************************************************************************************************************************************************************************************************	
