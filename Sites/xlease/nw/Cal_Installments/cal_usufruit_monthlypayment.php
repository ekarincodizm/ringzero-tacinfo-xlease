	...
	<table width="350" border="0" cellspacing="1" cellpadding="1" align="center" name="cal1" id="cal1">
		<tr>
				...
				<tr>
					...
					<td>
						<select name="nw_province">
						<?php 
							include("../../config/config.php");
							$qry_prov = pg_query("select * from \"price_messenger_rate\" ORDER BY \"pmr_destination\""); 
							while($re_prov = pg_fetch_array($qry_prov)){
								if($re_prov["pmr_destination"] == 'กรุงเทพมหานคร'){ $selected = 'selected'; }else{ $selected =""; }
								echo "<option value=\"".$re_prov["pmr_serial"]."\" $selected>".$re_prov["pmr_destination"]."</option>";
							}
						?>
						</select>
					</td>
